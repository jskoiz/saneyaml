...
- Ecosystem divergence target: libyaml/YAML 1.1-era behavior is documented and
  ...
  `saneyaml::Timestamp` typed reads while keeping byte payloads tagged unless the
...
| Emission | `EmitOptions::structural()` produces deterministic structural YAML for emittable trees; duplicate-effective mapping keys, over-depth trees including caller-built complex keys, and directly nested tags are rejected before output; public writers follow `serde_yaml` document-marker policy by omitting `---` for the first ordinary document and inserting `---` between stream documents; emitter round-trip fuzz covers `to_string`, `to_writer`, streaming `Serializer`, and opt-in byte-compatible value output. Structural options can preserve or sort mapping keys, use plain-where-safe/single/double quoted scalars, emit literal or folded block scalars where representable, and choose block or flow collections. `EmitOptions::with_yaml_1_1_safe_strings(true)` additionally quotes plain strings that YAML 1.2 keeps as strings but YAML 1.1 / `serde_yaml`-style readers would resolve to booleans or numbers (`no`, `yes`, `on`, `off`, sexagesimals, and octal/hex/binary integers), so emitted strings round-trip through YAML 1.1 consumers as well. `EmitOptions::byte_compatible()` matches `serde_yaml` bytes for common Serde structural values, enum tags, mapping-value sequences, typed real-world config writer shapes, and bytes rejection; comments, source style, graph identity, directives, anchors/aliases, and arbitrary lossless formatting remain out of this tier. | Manual comparison only | Manual comparison only | Public writer document-marker policy is matched; byte-compatible parity is covered for the supported structural writer corpus, not for arbitrary source-preserving YAML |
...
- `saneyaml::from_str<T>(&str) -> Result<T>`
- `saneyaml::from_slice<T>(&[u8]) -> Result<T>`
- `saneyaml::from_reader<T, R: std::io::Read>(R) -> Result<T>`
- `saneyaml::from_node<T>(&Node) -> Result<T>`
- `saneyaml::from_documents_str<T>(&str) -> Result<Vec<T>>`
- `saneyaml::from_documents_slice<T>(&[u8]) -> Result<Vec<T>>`
- `saneyaml::from_documents_reader<T, R: std::io::Read>(R) -> Result<Vec<T>>`
- `saneyaml::from_value<T>(saneyaml::Value) -> Result<T>`
- `saneyaml::Deserializer::{from_str, from_slice, from_reader}` for iterating
  multi-document streams as standard Serde document deserializers, and for
  single-document `T::deserialize(saneyaml::Deserializer::from_str(input))`
- `saneyaml::with::singleton_map::deserialize` for read-side enum field
  annotations compatible with `serde_yaml::with::singleton_map`
- `saneyaml::with::singleton_map_recursive::deserialize` for read-side nested enum
...
- `saneyaml::to_value<T: serde::Serialize>(T) -> Result<saneyaml::Value>` for
  common config-shaped structs, maps, sequences, scalar values, and Serde enum
  representations
- `saneyaml::to_string<T: serde::Serialize>(&T) -> Result<String>` and
  `saneyaml::to_writer<W: std::io::Write, T: serde::Serialize>(W, &T)` for
  deterministic structural emission from serializable config-shaped values
- `saneyaml::to_string_with_options`, `saneyaml::to_writer_with_options`, and
  `saneyaml::EmitOptions` for explicit emission controls. `EmitOptions::structural()`
...
- `saneyaml::Serializer<W: std::io::Write>` with `new`, `flush`, and `into_inner`
  ...
  tiers.
- `saneyaml::value::Serializer` and `saneyaml::value::to_value` for
  `serde_yaml::value`-style value serialization paths
- `saneyaml::with::singleton_map::serialize` and
  `saneyaml::with::singleton_map_recursive::serialize` for write-side enum field
  annotations compatible with the corresponding `serde_yaml::with` helpers
- `saneyaml::parse_str`, `parse_bytes`, `parse_documents`, and `parse_events`
- `saneyaml::stream::{EventStream, DocumentStream}` plus
  `saneyaml::stream_events*` and `saneyaml::stream_documents*` root helpers for
  pull-based parser events and one-document-at-a-time loading
- `saneyaml::parse_lossless`, `parse_lossless_bytes`, `saneyaml::LosslessStream`, and
  `saneyaml::LosslessEdit` for source-backed comment/trivia preservation,
  ...
  `saphyr`
- `saneyaml::LoadOptions::{new, core, json, failsafe, yaml_1_1,
  ...
  stream_documents_slice, stream_documents_reader}` and `saneyaml::Schema` for
...
- `saneyaml::Error` keeps its default flat `Display` string compatible with the
...
| `to_string`, `to_writer`, `Serializer` | `saneyaml::to_string`, `saneyaml::to_writer`, `saneyaml::Serializer`, optioned writer paths | `EmitOptions::structural()` writer support covered as the default; `byte_compatible()` byte parity covered for the supported structural writer corpus; structural style knobs are opt-in |
...
`saneyaml::Value` is a spanless read-side Serde value, matching the replacement
...
YAML 1.1 scalar behavior can opt in with `saneyaml::LoadOptions::yaml_1_1()`,
`saneyaml::LoadOptions::legacy_serde_yaml()`, or
`saneyaml::LoadOptions::new().schema(Schema::LegacySerdeYaml)`; the retained
`Schema::Yaml11` spelling is behavior-equivalent.
Callers that want document headers to select scalar construction can use
`saneyaml::LoadOptions::yaml_version_directive()` or
...
`saneyaml::Index` and `saneyaml::mapping::Index` are sealed preview traits; use the
built-in string, `usize`, and `Value` lookup surfaces instead of implementing
custom index types. Parser spans remain on
`saneyaml::Node`, whose recursive tree payload is `saneyaml::NodeValue`; typed
...
`Deserialize::deserialize(saneyaml::Deserializer::from_str(input))` preserves the
...
`saneyaml::Deserializer::from_str(&str)`,
`saneyaml::Deserializer::from_slice(&[u8])`, `saneyaml::from_node(&Node)`,
stream items from `saneyaml::Deserializer`, and `Deserialize::deserialize(&Value)` can
...
`saneyaml::Deserializer` yields successfully parsed documents before the first
later parser error item in a stream; the `from_documents_*` helpers remain
all-or-error convenience APIs.
For empty input, direct `saneyaml::from_str::<Value>("")`,
...
The writer serializer, `saneyaml::to_value`, `saneyaml::to_string`, and
`saneyaml::to_writer` are structural write-side bridges for replacement code that
...
the parser/emitter round-trip invariant. `saneyaml::to_value` and
`saneyaml::value::Serializer` match `serde_yaml` for generic 128-bit integer
...
Parser-backed `saneyaml::Value` reads still retain widened `i128`/`u128` numbers.
`saneyaml::to_string`, `saneyaml::to_writer`, and
`saneyaml::Serializer<W>` omit an explicit `---` for the first ordinary document and
...
`saneyaml::Deserializer::from_reader` is likewise owned-only for borrowed `&str`
targets.
...
semantic loaded tree or universal schema agreement. Use `saneyaml::LosslessStream`
...
- typed integer reads support `i128` and `u128` targets, including scalar
  ...
  is needed.
- `saneyaml::Number` stores signed and unsigned integers as `i128`/`u128`;
...
- `saneyaml::Number` follows Rust `f64` equality for finite float identity in
...
- generic `Serialize` inputs to `saneyaml::to_value` and `saneyaml::value::Serializer`
  follow `serde_yaml::value::Serializer` by stringifying 128-bit integers that
  do not fit in `i64` or `u64`; parsed `saneyaml::Value` trees keep widened
  `Number` values.
- `saneyaml::Value` and `saneyaml::Number` expose the common `serde_yaml` read-side
...
- `saneyaml::mapping` exposes `serde_yaml`-style public iterator names
