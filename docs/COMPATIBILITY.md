...
- `saneyaml::to_string_with_options`, `saneyaml::to_writer_with_options`, and
  ...
  ordering, Serde enum representation, scalar quote style, block scalar style,
  and collection layout. `EnumRepresentation::Tag` is the default writer shape;
  `EnumRepresentation::SingletonMap` is an opt-in global writer shape for
  migration paths that want `serde_yaml::with::singleton_map` output without
  annotating every enum field. `to_value` remains a tagged-value path.
