...
use serde::Serialize;
use saneyaml::{BlockScalarStyle, EmitCollectionStyle, EmitOptions, Node, ScalarQuoteStyle, Value};

fuzz_target!(|input: &[u8]| {
    let Ok(node) = saneyaml::parse_bytes(input) else {
        return;
    };
...
fn assert_emit_roundtrip_invariants(node: &Node) {
    ...
    saneyaml::to_writer_with_options(&mut byte_value_written, &value, EmitOptions::byte_compatible())
    ...
    let byte_reparsed_value: Value =
        saneyaml::from_str(&byte_value_emitted).expect("parse byte-compatible emitted value");
    ...
    let mut byte_stream =
        saneyaml::Serializer::with_options(Vec::new(), EmitOptions::byte_compatible());
    value
    ...
    let byte_stream_values =
        saneyaml::from_documents_str::<Value>(&byte_stream_output)
...
fn assert_optioned_value_roundtrip(value: &Value, options: EmitOptions) {
    let emitted =
        saneyaml::to_string_with_options(value, options).expect("emit optioned parsed value");
    let mut written = Vec::new();
    ...
    let emitted_again =
        saneyaml::to_string_with_options(&reparsed, options).expect("re-emit optioned parsed value");
    assert_eq!(emitted_again, emitted);
}
