...
use saneyaml::{CollectionStyle, Error, LosslessNodeKind, LosslessStream, NodeId, Span};
...
fn assert_structural_lossless_edit_invariants(
    ...
    match edit.finish() {
        Ok(output) => {
            saneyaml::parse_lossless(&output).expect("structural edit output reparses losslessly");
        }
        Err(error) => {
...
fn select_target(stream: &saneyaml::LosslessStream, input: EditInput<'_>) -> Option<EditTarget> {
    match input.mode {
