...
use saneyaml::{Error, Node, NodeValue, Span};
...
fn assert_success_invariants(node: &Node) {
    ...
    let direct = saneyaml::Value::from(node);
    let from_node: saneyaml::Value = saneyaml::from_node(node).expect("from_node value");
    let from_value: saneyaml::Value = saneyaml::from_value(direct.clone()).expect("from_value value");
