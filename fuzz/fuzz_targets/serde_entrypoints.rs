...
use saneyaml::{Error, LoadOptions, Span, Value};
...
fn assert_single_document_entrypoint(input: &[u8]) {
    let result = saneyaml::from_slice::<Value>(input);
    match result {
        Ok(value) => {
...
fn assert_document_stream_entrypoints(input: &[u8]) {
    ...
    match &from_documents {
        Ok(values) => {
            ...
            let nodes = saneyaml::parse_documents(input).expect("document success must parse");
...
fn assert_borrowed_entrypoints(input: &[u8]) {
    ...
    for result in saneyaml::Deserializer::from_slice(input)
