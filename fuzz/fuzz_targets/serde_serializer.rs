...
where
    T: Serialize,
{
    let expected = yaml::to_value(value).expect("value serializer accepts shape");
    let emitted = yaml::to_string(value).expect("document writer accepts shape");

    let mut written = Vec::new();
    yaml::to_writer(&mut written, value).expect("writer accepts shape");
    assert_eq!(written, emitted.as_bytes());

    let reparsed: Value = yaml::from_str(&emitted).expect("document writer output reparses");
    assert!(
...
where
    ...
    B: Serialize,
{
    let expected_first = yaml::to_value(first).expect("first value serializes");
    let expected_second = yaml::to_value(second).expect("second value serializes");
    ...
    let docs: Vec<Value> = yaml::from_documents_str(&output).expect("stream output reparses");
