...
//!
//! ```rust
//! let mut mapping = saneyaml::Mapping::new();
//! mapping.insert(
//!     saneyaml::Value::String("name".to_owned()),
//!     saneyaml::Value::String("api".to_owned()),
//! );
//! let value = saneyaml::Value::Mapping(mapping);
//!
//! let output = saneyaml::to_string_with_options(
//!     &value,
//!     saneyaml::EmitOptions::structural().with_key_order(saneyaml::KeyOrder::Sort),
//! )?;
//! assert!(output.contains("name: api"));
//!
//! let mut bytes = Vec::new();
//! saneyaml::to_writer(&mut bytes, &value)?;
