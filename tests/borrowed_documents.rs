...
fn mapping_value<'a, 'de>(
    node: &'a yaml::BorrowedNode<'de>,
    key: &str,
) -> &'a yaml::BorrowedNode<'de> {
    let yaml::BorrowedNodeValue::Mapping(entries) = &node.value else {
...
fn assert_borrowed_string(node: &yaml::BorrowedNode<'_>, expected: &str) {
    let yaml::BorrowedNodeValue::String(Cow::Borrowed(actual)) = &node.value else {
        panic!("expected borrowed string, got {:?}", node.value);
    };
    assert_eq!(*actual, expected);
}

fn assert_owned_string(node: &yaml::BorrowedNode<'_>, expected: &str) {
    let yaml::BorrowedNodeValue::String(Cow::Owned(actual)) = &node.value else {
...
fn borrowed_documents_borrow_only_sliceable_scalars() {
    ...
    let docs = yaml::parse_borrowed_documents(input).expect("borrowed documents");
    let doc = &docs[0];
...
fn borrowed_documents_match_owned_tree_after_merges() {
    ...
    let owned = yaml::parse_documents(input).expect("owned documents");
    let borrowed = yaml::parse_borrowed_documents(input).expect("borrowed documents");
    ...
    assert!(
        borrowed[0]
            .clone()
            .into_owned_value()
            .equivalent(&yaml::Value::from(&owned[0]))
    );
...
fn borrowed_documents_respect_load_options() {
    let input = "%YAML 1.1\n---\nflag: YES\n";
    let docs = yaml::LoadOptions::yaml_version_directive()
        .parse_borrowed_documents(input)
        .expect("directive-driven borrowed documents");
    let flag = mapping_value(&docs[0], "flag");
    assert!(matches!(flag.value, yaml::BorrowedNodeValue::Bool(true)));
}

#[test]
fn borrowed_documents_keep_parser_diagnostics() {
    let input = "a: 1\na: 2\n";
    let owned_error = yaml::parse_documents(input).expect_err("owned duplicate key");
    let borrowed_error = yaml::parse_borrowed_documents(input).expect_err("borrowed duplicate key");
