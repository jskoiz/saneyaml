use saneyaml::{LoadOptions, parse_lossless, parse_str};
use saphyr::LoadableYamlNode;

#[test]
fn divergence_yaml_1_1_boolean_words_are_strings() {
    let node = parse_str("on: off\nyes: no\nY: N\n").expect("parse");
    let saneyaml::NodeValue::Mapping(entries) = node.value else {
...
fn divergence_yaml_1_1_version_directive_switching_requires_explicit_option() {
    ...
    let saneyaml::NodeValue::Mapping(entries) = default.value else {
        panic!("expected mapping");
    };
    ...
    let saneyaml::NodeValue::Mapping(entries) = directive.value else {
        panic!("expected directive-driven mapping");
    };
    assert!(matches!(
        entries[0].1.value,
        saneyaml::NodeValue::Bool(true)
    ));
}
...
fn divergence_merge_keys_expand_by_default_in_loaded_trees() {
    ...
    let saneyaml::NodeValue::Mapping(job) = &entries[1].1.value else {
    ...
    assert!(
        job.iter()
            .any(|(key, value)| key.as_str() == Some("retries")
                && matches!(
                    value.value,
                    saneyaml::NodeValue::Number(saneyaml::Number::Integer(3))
                )),
...
fn divergence_merge_list_expands_by_default_in_loaded_trees() {
    ...
    let saneyaml::NodeValue::Mapping(merged) = &entries[2].1.value else {
...
fn divergence_merge_key_edges_keep_default_strict_and_yaml11_compatible() {
    ...
    let recovered = saneyaml::Value::from(recovered);
...
fn divergence_custom_tags_are_preserved_without_schema_coercion() {
    ...
    let saneyaml::NodeValue::Tagged(tagged) = &entries[0].1.value else {
        panic!("expected tagged value");
    };
    assert_eq!(tagged.tag, saneyaml::Tag::new("Thing"));
...
fn divergence_block_merge_key_expands_by_default() {
    ...
    let saneyaml::NodeValue::Mapping(job) = &entries[0].1.value else {
        panic!("expected job mapping");
    };
...
fn divergence_default_schema_keeps_legacy_dates_and_sexagesimal_as_strings() {
    ...
    assert!(matches!(
        entries[1].1.value,
        saneyaml::NodeValue::Number(saneyaml::Number::Integer(123))
    ));
...
fn divergence_yaml11_timestamps_have_native_typed_api() {
    ...
    let saneyaml::NodeValue::Mapping(default_entries) = default.value else {
    ...
    let saneyaml::NodeValue::Mapping(entries) = yaml11.value else {
    ...
    for (value, source) in [
        ...
        let saneyaml::NodeValue::Tagged(tagged) = &value.value else {
            panic!("expected timestamp tag");
        };
        assert_eq!(tagged.tag, saneyaml::Tag::new("!!timestamp"));
        assert_eq!(tagged.value.as_str(), Some(source));
        assert_eq!(
            value.as_timestamp(),
            saneyaml::Timestamp::parse_yaml_1_1(source)
        );
    }
}
...
fn divergence_legacy_scalar_resolution_record_is_present() {
    ...
    assert!(record.contains("exposed through the native saneyaml::Timestamp API"));
    assert!(
...
fn divergence_explicit_core_tags_record_is_present() {
    ...
    assert!(record.contains("explicit !!timestamp saneyaml::Timestamp reads"));
    assert!(record.contains("decision"));
}
...
fn divergence_explicit_core_tags_reference_matrix_matches_record() {
    ...
    let ours: saneyaml::Value = saneyaml::from_str(input).expect("ours explicit core tags");
    assert_eq!(
        ours["payload"].as_tagged().expect("payload tag").tag,
        saneyaml::Tag::new("!!binary")
    );
    assert_eq!(
        ours["value"].as_tagged().expect("int tag").tag,
        saneyaml::Tag::new("!!int")
    );
    assert_eq!(
        ours["date"].as_tagged().expect("timestamp tag").tag,
        saneyaml::Tag::new("!!timestamp")
    );
    assert_eq!(
    ...
    let ours_bool_on: bool =
        saneyaml::from_str("!!bool ON\n").expect("ours explicit YAML 1.1 bool tag");
    assert!(ours_bool_on);
    let canonical_bool_on: bool = saneyaml::from_str("!<tag:yaml.org,2002:bool> ON\n")
        .expect("ours canonical explicit YAML 1.1 bool tag");
    assert!(canonical_bool_on);
    let canonical_value: saneyaml::Value =
        saneyaml::from_str("!<tag:yaml.org,2002:int> 0x7B\n").expect("canonical int value");
...
";
    ...
    let ours: Typed = saneyaml::from_str(input).expect("ours YAML 1.1 collection tags");
    assert_eq!(
        ours.set,
    ...
    let retained: saneyaml::Value =
        saneyaml::from_str(input).expect("ours retained collection tags");
    assert_eq!(
        retained["set"].as_tagged().expect("set tag").tag,
        saneyaml::Tag::new("!!set")
    );
    assert_eq!(
        retained["omap"].as_tagged().expect("omap tag").tag,
        saneyaml::Tag::new("!!omap")
    );
    assert_eq!(
        retained["pairs"].as_tagged().expect("pairs tag").tag,
        saneyaml::Tag::new("!!pairs")
    );
...
";
    ...
    let ours: Typed = saneyaml::from_str(input).expect("ours core structural tags");
    ...
    assert_eq!(
        retained["resolved_seq"]
            ...
            .tag,
        saneyaml::Tag::new("!<tag:yaml.org,2002:seq>")
    );
...
fn divergence_flow_collection_comment_separation_rejects_yaml_suite_error() {
    for (name, input) in [
        ...
        if saneyaml::parse_events(input).is_ok() {
...
fn divergence_null_like_string_targets_record_is_present() {
    ...
    assert!(record.contains("saneyaml::Value"));
...
fn divergence_complex_collection_duplicate_key_policy_matches_documented_split() {
    ...
    saneyaml::parse_events(input).expect("raw event parser preserves X38W alias events");
    ...
    saneyaml::parse_events(permuted).expect("raw event parser preserves permuted duplicate keys");
}
...
fn divergence_duplicate_scalar_key_policy_matches_config_safety_decision() {
    ...
    saneyaml::parse_events(input).expect("raw event parser exposes duplicate keys");
}
...
#[test]
fn divergence_yaml_suite_flow_collection_shared_deferrals_record_is_present() {
    let record = include_str!(
        "fixtures/divergences/records/yaml-suite-flow-collection-shared-deferrals.toml"
    );
    assert!(record.contains("yaml-suite-flow-collection-shared-deferrals"));
    for case in [
        "4ABK", "4MUZ/00", "4MUZ/01", "4MUZ/02", "DBG4", "FRK4", "HM87/00", "K3WX", "NJ66", "NKF9",
        "VJP3/01",
    ] {
        assert!(record.contains(case));
    }
    assert!(record.contains("flow collection"));
    assert!(record.contains("serde_yaml 0.9.34"));
    assert!(record.contains("yaml-rust2 0.11.0"));
    assert!(record.contains("saphyr 0.0.6"));
    assert!(record.contains("decision"));
...
#[test]
fn divergence_yaml_suite_stream_marker_shared_deferrals_record_is_present() {
    let record =
        include_str!("fixtures/divergences/records/yaml-suite-stream-marker-shared-deferrals.toml");
    assert!(record.contains("yaml-suite-stream-marker-shared-deferrals"));
    for case in ["7Z25", "HWV9", "NHX8", "QT73", "SM9W/01"] {
        assert!(record.contains(case));
    }
    assert!(record.contains("document-end"));
    assert!(record.contains("serde_yaml 0.9.34"));
    assert!(record.contains("yaml-rust2 0.11.0"));
    assert!(record.contains("saphyr 0.0.6"));
    assert!(record.contains("decision"));
...
#[test]
fn divergence_yaml_suite_anchor_character_shared_deferrals_record_is_present() {
    let record = include_str!(
        "fixtures/divergences/records/yaml-suite-anchor-character-shared-deferrals.toml"
    );
    assert!(record.contains("yaml-suite-anchor-character-shared-deferrals"));
    for case in ["8XYN", "W5VH"] {
        assert!(record.contains(case));
    }
    assert!(record.contains("Unicode anchors"));
    assert!(record.contains("serde_yaml 0.9.34"));
    assert!(record.contains("yaml-rust2 0.11.0"));
    assert!(record.contains("saphyr 0.0.6"));
    assert!(record.contains("decision"));
...
#[test]
fn divergence_yaml_suite_block_indentation_shared_deferrals_record_is_present() {
    let record = include_str!(
        "fixtures/divergences/records/yaml-suite-block-indentation-shared-deferrals.toml"
    );
    assert!(record.contains("yaml-suite-block-indentation-shared-deferrals"));
    assert!(record.contains("A2M4"));
    assert!(record.contains("indentation-indicator"));
    assert!(record.contains("serde_yaml 0.9.34"));
    assert!(record.contains("yaml-rust2 0.11.0"));
    assert!(record.contains("saphyr 0.0.6"));
    assert!(record.contains("decision"));
...
fn divergence_typed_scalar_mapping_keys_are_distinct() {
    ...
    assert!(matches!(entries[4].0.value, saneyaml::NodeValue::Null));
...
fn divergence_numeric_forms_are_documented_policy() {
    ...
    assert!(matches!(
        entries[5].1.value,
        saneyaml::NodeValue::Number(saneyaml::Number::Float(value))
            if value == f64::INFINITY
    ));
    assert!(matches!(
        entries[6].1.value,
        saneyaml::NodeValue::Number(saneyaml::Number::Float(value)) if value.is_nan()
    ));
    assert!(matches!(
        entries[7].1.value,
        saneyaml::NodeValue::Number(saneyaml::Number::Integer(1000))
    ));
    assert!(matches!(
        entries[8].1.value,
        saneyaml::NodeValue::Number(saneyaml::Number::Float(value)) if value == 12.34
    ));
}
...
fn divergence_supported_directives_are_syntax_only_and_reserved_directives_are_ignored() {
    ...
    let saneyaml::NodeValue::Mapping(entries) = yaml.value else {
        panic!("expected mapping");
    };
    ...
    let saneyaml::NodeValue::Mapping(entries) = yaml_1_1.value else {
        panic!("expected YAML 1.1 mapping");
    };
    assert_eq!(entries[0].0.as_str(), Some("on"));
    assert_eq!(entries[0].1.as_str(), Some("off"));
    ...
    let saneyaml::NodeValue::Mapping(entries) = tagged.value else {
        panic!("expected mapping");
    };
    let saneyaml::NodeValue::Tagged(tagged) = &entries[0].1.value else {
        panic!("expected tagged value");
...
fn divergence_undeclared_named_tag_handles_are_rejected() {
    for input in [
        ...
        let event_error =
            saneyaml::parse_events(input).expect_err("events reject undeclared handle");
        assert!(
            event_error
                .to_string()
                .contains("undeclared TAG directive handle")
...
fn divergence_binary_and_core_tags_are_preserved_in_retained_trees() {
    ...
    let saneyaml::NodeValue::Tagged(binary) = &entries[0].1.value else {
        panic!("expected binary tag");
    };
    assert_eq!(binary.tag, saneyaml::Tag::new("!!binary"));
    assert_eq!(binary.value.as_str(), Some("SGVsbG8="));
    let decoded: Vec<u8> = saneyaml::from_str("!!binary SGVsbG8=\n")
        .expect("typed byte target decodes explicit binary");
    assert_eq!(decoded, b"Hello");
    let explicit_timestamp: saneyaml::Timestamp =
        saneyaml::from_str("!!timestamp 2026-05-24\n").expect("explicit timestamp typed read");
    assert_eq!(
        explicit_timestamp,
        saneyaml::Timestamp::parse_yaml_1_1("2026-05-24").expect("timestamp")
    );

    let saneyaml::NodeValue::Tagged(integer) = &entries[1].1.value else {
        panic!("expected int tag");
    };
    assert_eq!(integer.tag, saneyaml::Tag::new("!!int"));
    assert_eq!(integer.value.as_str(), Some("0x7B"));

    let saneyaml::NodeValue::Tagged(timestamp) = &entries[2].1.value else {
        panic!("expected timestamp tag");
    };
    assert_eq!(timestamp.tag, saneyaml::Tag::new("!!timestamp"));
    ...
    let saneyaml::NodeValue::Tagged(float) = &entries[3].1.value else {
        panic!("expected float tag");
    };
    assert_eq!(float.tag, saneyaml::Tag::new("!!float"));
