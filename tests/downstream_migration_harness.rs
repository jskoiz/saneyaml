...
fn downstream_kubernetes_stream_matches_serde_yaml() {
    ...
    let parsed: Vec<KubernetesDoc> = saneyaml::from_documents_str(input).expect("yaml k8s stream");
...
fn downstream_kubernetes_crd_schema_matches_serde_yaml() {
    ...
    let parsed: Vec<serde_json::Value> =
        saneyaml::from_documents_str(input).expect("yaml k8s crd stream");
    let reference = serde_yaml::Deserializer::from_str(input)
...
fn downstream_openapi_polymorphism_matches_serde_yaml() {
    ...
    let parsed: serde_json::Value = saneyaml::from_str(input).expect("yaml openapi");
...
fn downstream_ansible_playbook_sequence_matches_serde_yaml() {
    ...
    let parsed: Vec<AnsiblePlay> = saneyaml::from_str(input).expect("yaml ansible playbook");
