use saneyaml::{CollectionStyle, Event, ScalarStyle, parse_events, parse_str};

fn scalar_events(
    events: &[Event],
) -> Vec<(&str, ScalarStyle, &saneyaml::EventMeta, saneyaml::Span)> {
    events
        .iter()
    ...
    assert!(events.iter().any(|event| {
        matches!(
            ...
            Event::Scalar {
                ...
                && meta.tag.as_ref().is_some_and(|tag| tag.tag == saneyaml::Tag::new("Thing"))
        )
    }));
}
...
fn event_policy_raw_events_accept_aliases_as_distinct_block_mapping_keys() {
    let input = "a: &a one\nb: &b two\n? *a\n: first\n? *b\n: second\n";
    saneyaml::parse_documents(input).expect("tree parser accepts scalar alias keys");
...
fn event_policy_raw_events_do_not_reject_duplicate_tree_keys() {
    ...
    saneyaml::parse_documents(block_input).expect_err("tree parser rejects duplicate keys");
    ...
    saneyaml::parse_documents(flow_input).expect_err("tree parser rejects duplicate flow keys");
    ...
    saneyaml::parse_documents(collection_input)
    ...
    saneyaml::parse_documents(tagged_input).expect_err("tree parser rejects tagged duplicate keys");
...
fn event_policy_raw_events_allow_recursive_aliases_without_expanding() {
    ...
    saneyaml::parse_documents(input).expect_err("tree parser rejects recursive alias expansion");
...
";
    saneyaml::parse_documents(input).expect("tree parser accepts flow key metadata");
