...
fn external_pingora_server_configs_match_serde_yaml() {
    for path in [
        ...
        let value = saneyaml::to_value(&parsed).expect("yaml to_value pingora config");
        ...
        let output = saneyaml::to_string(&parsed).expect("yaml to_string pingora config");
        let reparsed: PingoraServerConf =
            saneyaml::from_str(&output).expect("yaml pingora output reparses");
...
{
    let parsed: T = saneyaml::from_str(input).expect("yaml downstream parse");
...
{
    let parsed: T =
        saneyaml::from_reader(Cursor::new(input.as_bytes())).expect("yaml downstream reader parse");
...
fn assert_value_matches_serde(input: &str) -> Value {
    let parsed: Value = saneyaml::from_str(input).expect("yaml downstream value parse");
...
fn assert_value_writer_replays(path: &str, value: &Value) {
    let emitted = saneyaml::to_string(value)
        .unwrap_or_else(|error| panic!("yaml writes downstream value {path}: {error}"));
    let reparsed: Value = saneyaml::from_str(&emitted)
    ...
    saneyaml::to_writer(&mut written, value)
    ...
    let byte_reparsed: Value = saneyaml::from_str(&byte_compatible).unwrap_or_else(|error| {
    ...
    let docs = saneyaml::Deserializer::from_str(&stream_output)
    ...
    let mut byte_stream =
        saneyaml::Serializer::with_options(Vec::new(), saneyaml::EmitOptions::byte_compatible());
    ...
    let byte_docs = saneyaml::Deserializer::from_str(&byte_stream_output)
        .map(Value::deserialize)
        .collect::<Result<Vec<_>, _>>()
        .unwrap_or_else(|error| {
