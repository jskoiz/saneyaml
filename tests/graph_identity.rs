...
second: *base
";
    let mut value: Value = saneyaml::from_str(input).expect("semantic value");
...
fn recursive_alias_identity_is_lossless_only_not_semantic_value_identity() {
    ...
    let error = saneyaml::from_str::<Value>(input).expect_err("semantic recursive alias rejected");
...
fn effective_entry<'a>(
    ...
    entries: &'a [saneyaml::LosslessEffectiveMappingEntry],
    key: &str,
    value: Option<&str>,
) -> &'a saneyaml::LosslessEffectiveMappingEntry {
    entries
        .iter()
        .find(|entry| {
...
fn normalize_lossless_graph(input: &str) -> saneyaml::Result<Vec<GraphOp>> {
    let stream = parse_lossless(input)?;
