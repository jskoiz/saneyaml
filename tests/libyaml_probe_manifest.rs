...
fn run_rust_probe_case(case: &Toml) -> RustProbe {
    ...
    match toml_str(case, "rust_entrypoint") {
        "default-value" => rust_probe_from_result(
            saneyaml::from_str::<saneyaml::Value>(&input).map(|value| format!("{value:#?}")),
        ),
        "yaml11-value" => rust_probe_from_result(
            saneyaml::LoadOptions::yaml_1_1()
                .from_str::<saneyaml::Value>(&input)
                .map(|value| format!("{value:#?}")),
        ),
        "directive-value" => rust_probe_from_result(
            saneyaml::LoadOptions::yaml_version_directive()
                .from_str::<saneyaml::Value>(&input)
                .map(|value| format!("{value:#?}")),
        ),
        "directive-stream" => rust_probe_from_result(
            saneyaml::LoadOptions::yaml_version_directive()
                .from_documents_str::<saneyaml::Value>(&input)
                .map(|value| format!("{value:#?}")),
        ),
        "typed-set-strings" => rust_probe_from_result(
            saneyaml::from_str::<BTreeSet<String>>(&input).map(|value| format!("{value:#?}")),
        ),
        "typed-string-i64-pairs" => rust_probe_from_result(
            saneyaml::from_str::<Vec<(String, i64)>>(&input).map(|value| format!("{value:#?}")),
        ),
        "typed-collection-bundle" => rust_probe_from_result(
            saneyaml::from_str::<ProbeCollectionBundle>(&input).map(format_probe_collection_bundle),
        ),
        "events" => rust_probe_from_result(
            saneyaml::parse_events(&input).map(|events| format!("{events:#?}")),
        ),
        "lossless" => rust_probe_from_result(
            saneyaml::parse_lossless(&input).map(|stream| format!("{stream:#?}")),
        ),
...
fn rust_probe_from_result(result: saneyaml::Result<String>) -> RustProbe {
    match result {
        Ok(output) => RustProbe {
            status: "ok",
            output,
