...
const PARSE_BYTES_REQUIRED_SEEDS: &[&str] = &[
    ...
    "yts-2auy-tags-in-block-sequence",
    "yts-33x3-explicit-int-sequence",
    "yts-4abk-flow-mapping-separate-values",
    "yts-4muz-00-flow-mapping-colon-after-key",
    "yts-4muz-01-flow-mapping-colon-after-key",
    "yts-4muz-02-flow-mapping-colon-after-key",
    "yts-6kgn-empty-anchor-alias",
    "yts-74h7-tags-in-implicit-mapping",
    "yts-7z25-bare-doc-after-end",
    "yts-8xyn-unicode-anchor",
    "yts-8g76-comment-lines",
    "yts-98yd-comment-only",
    "yts-a2m4-indentation-indicators",
    "yts-avm7-empty-stream",
    "yts-c4hz-global-tags-anchor-alias",
    "yts-dbg4-plain-characters",
    "yts-f2c7-anchors-and-tags",
    "yts-fh7j-tags-on-empty-scalars",
    "yts-frk4-empty-flow-nodes",
    "yts-hm87-00-flow-syntax-char",
    "yts-hwv9-document-end-only",
    "yts-k3wx-comment-colon-flow-mapping",
    "yts-k54u-tab-after-document-header",
    "yts-l94m-tags-in-explicit-mapping",
    "yts-nhx8-empty-lines-end-document",
    "yts-nj66-multiline-plain-flow-key",
    "yts-nkf9-empty-keys-block-flow",
    "yts-pw8x-anchors-on-empty-scalars",
    "yts-qt73-comment-document-end",
    "yts-s4jq-explicit-non-specific-tag",
    "yts-sm9w-01-empty-key",
    "yts-vjp3-01-multiline-flow-collection",
...
const LOSSLESS_GRAPH_REQUIRED_SEEDS: &[&str] = &[
    ...
    "yts-c4hz-global-tags-anchor-alias.yml",
    "yts-cn3r-flow-anchor-properties.yml",
    "yts-cup7-tagged-anchor-alias.yml",
    "yts-e76z-anchor-alias-keys.yml",
    "yts-f2c7-anchors-and-tags.yml",
    "yts-fh7j-tags-on-empty-scalars.yml",
    "yts-pw8x-anchors-on-empty-scalars.yml",
    "yts-8xyn-unicode-anchor.yml",
    "yts-w5vh-alias-allowed-characters.yml",
...
proptest! {
    ...
    fn parser_never_panics_on_bytes(input in proptest::collection::vec(any::<u8>(), 0..4096)) {
        let result = std::panic::catch_unwind(|| saneyaml::parse_bytes(&input))
    ...
    fn invalid_utf8_diagnostics_stay_in_bounds(prefix in proptest::collection::vec(prop_oneof![
        ...
        std::panic::catch_unwind(|| {
            let error = saneyaml::parse_bytes(&input).expect_err("inserted invalid UTF-8");
            let span = error.span();
    ...
    fn parser_never_panics_on_utf8(input in "\\PC{0,4096}") {
        let result = std::panic::catch_unwind(|| saneyaml::parse_str(&input))
            .expect("parser must not panic");
    ...
    fn event_stream_never_panics_on_utf8(input in "\\PC{0,4096}") {
        let result = std::panic::catch_unwind(|| saneyaml::parse_events(&input))
    ...
    fn pull_event_stream_matches_batch_events(input in "\\PC{0,4096}") {
        let batch = saneyaml::parse_events(&input);
        let streamed = saneyaml::EventStream::from_str(&input)
    ...
    fn pull_document_stream_matches_batch_documents(input in "\\PC{0,4096}") {
        let batch = saneyaml::parse_documents(&input);
    ...
    fn emitted_generated_trees_parse_equivalently(node in arb_node(4)) {
        ...
        let emitted = match saneyaml::to_string(&node) {
            Ok(emitted) => emitted,
        ...
        let reparsed = saneyaml::parse_str(&emitted).expect("parse emitted node");
        prop_assert!(reparsed.equivalent(&node));
        let emitted_again = saneyaml::to_string(&reparsed).expect("emit reparsed node");
        prop_assert_eq!(emitted_again, emitted);
    }

    #[test]
    fn from_node_and_from_value_agree_for_generated_values(node in arb_node(4)) {
        let from_node: Value = saneyaml::from_node(&node).expect("from_node value");
        let from_value: Value = saneyaml::from_value(Value::from(node.clone())).expect("from_value value");
...
fn parser_corpus_does_not_panic() {
    ...
    {
        std::panic::catch_unwind(|| {
            let result = saneyaml::parse_bytes(&input);
            assert_parse_result_invariants(&input, &result);
        })
        ...
        .chain(synthetic_corpus_inputs())
    {
        std::panic::catch_unwind(|| {
            if let Ok(text) = std::str::from_utf8(&input) {
                let result = saneyaml::parse_events(text);
...
fn streaming_parse_fuzz_corpus_matches_batch_entrypoints() {
    for (name, input) in fuzz_corpus_inputs("streaming_parse") {
        std::panic::catch_unwind(|| {
            if let Ok(text) = std::str::from_utf8(&input) {
                let events = saneyaml::EventStream::from_str(text)
                    .and_then(|stream| stream.collect::<saneyaml::Result<Vec<_>>>());
                assert_eq!(events, saneyaml::parse_events(text), "{name} event stream");

                let documents = saneyaml::DocumentStream::from_str(text)
                    .and_then(|stream| stream.collect::<saneyaml::Result<Vec<_>>>());
                assert_eq!(
                    documents,
                    saneyaml::parse_documents(text),
...
fn emit_roundtrip_fuzz_corpus_emits_stably() {
    for (name, input) in fuzz_corpus_inputs("emit_roundtrip") {
        std::panic::catch_unwind(|| {
            if let Ok(node) = saneyaml::parse_bytes(&input) {
...
fn fuzz_corpora_cover_release_targets_and_named_safety_seeds() {
    let expected: BTreeMap<&str, (usize, &[&str])> = BTreeMap::from([
        ...
        ("lossless_graph", (35, LOSSLESS_GRAPH_REQUIRED_SEEDS)),
        ("parse_bytes", (914, PARSE_BYTES_REQUIRED_SEEDS)),
...
fn malformed_block_scalar_header_corpus_rejects_with_in_bounds_spans() {
    for (name, input) in malformed_block_scalar_header_inputs() {
        for error in [
            saneyaml::parse_str(input)
                .expect_err("parse_str rejects malformed block scalar header"),
...
fn duplicate_tree_key_corpus_rejects_tree_loading_with_in_bounds_spans() {
    for (name, input) in duplicate_tree_key_inputs() {
        saneyaml::parse_events(input).expect("raw events preserve duplicate collection keys");
        for error in [
            saneyaml::parse_str(input).expect_err("parse_str rejects duplicate tree key"),
            saneyaml::from_str::<Value>(input).expect_err("from_str rejects duplicate tree key"),
        ] {
            assert!(
...
fn assert_lossless_graph_invariants(input: &[u8]) {
    let result = saneyaml::parse_lossless_bytes(input);
    if let Ok(input_str) = std::str::from_utf8(input)
        && saneyaml::parse_events(input_str).is_ok()
    {
        assert!(
            result.is_ok(),
...
fn assert_lossless_edit_invariants(input: &[u8]) {
    ...
    match edit.finish() {
        Ok(output) => {
            ...
            saneyaml::parse_lossless(&output).expect("successful edit output reparses losslessly");
        }
...
fn assert_structural_lossless_edit_invariants(
    ...
    match edit.finish() {
        Ok(output) => {
            let edited =
                saneyaml::parse_lossless(&output).expect("structural edit output reparses");
...
fn assert_apply_merge_semantics(input: &[u8]) {
    ...
    match (value.apply_merge(), reference.apply_merge()) {
        (Ok(()), Ok(())) => {
            let reference = saneyaml::to_value(reference)
                .expect("serde_yaml value converts to saneyaml::Value");
            ...
            let repeated_reference = saneyaml::to_value(repeated_reference)
                .expect("repeated serde_yaml value converts to saneyaml::Value");
...
fn assert_single_document_entrypoint(input: &[u8]) {
    match saneyaml::from_slice::<Value>(input) {
        Ok(value) => {
            let node = saneyaml::parse_bytes(input).expect("from_slice success must parse");
            assert!(Value::from(&node).equivalent(&value));
        }
        Err(error) => assert_error_invariants(input, &error),
    }
...
fn assert_document_stream_entrypoints(input: &[u8]) {
    let from_documents = saneyaml::from_documents_slice::<Value>(input);
    match &from_documents {
        Ok(values) => {
            let input_text = std::str::from_utf8(input).expect("document success must be UTF-8");
            let nodes = saneyaml::parse_documents(input_text).expect("document success must parse");
    ...
    let stream_results = saneyaml::Deserializer::from_slice(input)
...
fn assert_reader_backed_entrypoints(input: &[u8]) {
    match (
        saneyaml::from_slice::<Value>(input),
        saneyaml::from_reader::<_, Value>(Cursor::new(input)),
    ) {
    ...
    let from_documents_slice = saneyaml::from_documents_slice::<Value>(input);
    let from_documents_reader = saneyaml::from_documents_reader::<Value, _>(Cursor::new(input));
    ...
    let reader_stream_results = saneyaml::Deserializer::from_reader(Cursor::new(input))
        .map(Value::deserialize)
        .collect::<Vec<_>>();
    match saneyaml::from_documents_reader::<Value, _>(Cursor::new(input)) {
    ...
    T: DeserializeOwned + std::fmt::Debug + PartialEq,
{
    assert_entrypoint_pair(
        ...
        saneyaml::from_slice::<T>(input),
        saneyaml::from_reader::<_, T>(Cursor::new(input)),
    );

    assert_entrypoint_pair(
        input,
        "direct slice/reader deserializer",
        T::deserialize(saneyaml::Deserializer::from_slice(input)),
        T::deserialize(saneyaml::Deserializer::from_reader(Cursor::new(input))),
    );

    assert_entrypoint_pair(
        input,
        "from_documents_slice/from_documents_reader",
        saneyaml::from_documents_slice::<T>(input),
        saneyaml::from_documents_reader::<T, _>(Cursor::new(input)),
    );
    ...
    assert_stream_results_match(
        ...
        saneyaml::Deserializer::from_slice(input)
            .map(T::deserialize)
            .collect::<Vec<_>>(),
        saneyaml::Deserializer::from_reader(Cursor::new(input))
            .map(T::deserialize)
            .collect::<Vec<_>>(),
    );
}
...
fn assert_entrypoint_pair<T>(
    input: &[u8],
    label: &str,
    left: saneyaml::Result<T>,
    right: saneyaml::Result<T>,
) where
...
fn assert_stream_results_match<T>(
    input: &[u8],
    left: Vec<saneyaml::Result<T>>,
    right: Vec<saneyaml::Result<T>>,
) where
    T: std::fmt::Debug + PartialEq,
{
...
fn assert_borrowed_entrypoints(input: &[u8]) {
    match saneyaml::from_slice::<BorrowedConfig<'_>>(input) {
        Ok(config) => {
    ...
    match saneyaml::from_slice::<BorrowedVars<'_>>(input) {
        Ok(config) => {
    ...
    match BorrowedConfig::deserialize(saneyaml::Deserializer::from_slice(input)) {
        Ok(config) => {
            assert_borrowed_from_input(input, config.name);
            assert_borrowed_from_input(input, config.path);
        }
    ...
    for result in saneyaml::Deserializer::from_slice(input).map(BorrowedConfig::deserialize) {
        match result {
            Ok(config) => {
                assert_borrowed_from_input(input, config.name);
                assert_borrowed_from_input(input, config.path);
...
fn assert_byte_entrypoints(input: &[u8]) {
    if let Err(error) = saneyaml::from_slice::<FuzzBytes>(input) {
        assert_error_invariants(input, &error);
    }

    if let Err(error) = FuzzBytes::deserialize(saneyaml::Deserializer::from_slice(input)) {
        assert_error_invariants_allowing_unspanned(input, &error);
    }

    for result in saneyaml::Deserializer::from_slice(input).map(FuzzBytes::deserialize) {
        if let Err(error) = result {
            assert_error_invariants(input, &error);
        }
    }
...
fn event_stream_fuzz_regression_closes_blank_separated_implicit_documents() {
    let input = "a-r--\n\noa: b\0\0\0\0\0\0\0\n";
    let events = saneyaml::parse_events(input).expect("events");
...
fn event_stream_root_scalar_regressions_stay_structurally_valid() {
    for (name, input) in [
        ...
        let result = saneyaml::parse_events(input);
        assert_event_result_invariants(input.as_bytes(), &result);
        result.unwrap_or_else(|error| panic!("event stream regression {name} failed: {error}"));
    }
}

fn assert_parse_result_invariants(input: &[u8], result: &saneyaml::Result<Node>) {
    match result {
        Ok(node) => {
...
fn assert_success_invariants(node: &Node) {
    ...
    let from_node: Value = saneyaml::from_node(node).expect("from_node value");
    let from_value: Value = saneyaml::from_value(direct.clone()).expect("from_value value");
    assert!(direct.equivalent(&from_node));
    assert!(direct.equivalent(&from_value));
}

fn assert_emit_roundtrip_invariants(node: &Node) {
    let emitted = saneyaml::to_string(node).expect("emit parsed tree");
    let mut written = Vec::new();
    saneyaml::to_writer(&mut written, node).expect("write parsed tree");
    assert_eq!(written, emitted.as_bytes());

    let reparsed = saneyaml::parse_str(&emitted).expect("parse emitted tree");
    assert!(
        reparsed.equivalent(node),
        "emitted YAML did not parse equivalently:\n{emitted}"
    );

    let emitted_again = saneyaml::to_string(&reparsed).expect("emit reparsed tree");
    assert_eq!(emitted_again, emitted);

    let value = Value::from(node);
    let value_emitted = saneyaml::to_string(&value).expect("emit parsed value");
    let mut value_written = Vec::new();
    saneyaml::to_writer(&mut value_written, &value).expect("write parsed value");
    assert_eq!(value_written, value_emitted.as_bytes());

    let reparsed_value: Value = saneyaml::from_str(&value_emitted).expect("parse emitted value");
    ...
    let stream_values =
        saneyaml::from_documents_str::<Value>(&stream_output).expect("parse streamed values");
    assert_eq!(stream_values.len(), 2);
    assert!(stream_values[0].equivalent(&value));
    assert!(stream_values[1].equivalent(&reparsed_value));
}

fn emit_option_roundtrip_matrix() -> Vec<saneyaml::EmitOptions> {
    let mut matrix = Vec::new();
    for key_order in [saneyaml::KeyOrder::Preserve, saneyaml::KeyOrder::Sort] {
        for scalar_quote_style in [
            saneyaml::ScalarQuoteStyle::PlainWhereSafe,
            saneyaml::ScalarQuoteStyle::SingleQuoted,
            saneyaml::ScalarQuoteStyle::DoubleQuoted,
        ] {
            for block_scalar_style in [
                saneyaml::BlockScalarStyle::Literal,
                saneyaml::BlockScalarStyle::Folded,
            ] {
                for collection_style in [
                    saneyaml::EmitCollectionStyle::Block,
                    saneyaml::EmitCollectionStyle::Flow,
                ] {
                    matrix.push(
                        saneyaml::EmitOptions::structural()
...
fn assert_optioned_value_roundtrip(value: &Value, options: saneyaml::EmitOptions) {
    let emitted = saneyaml::to_string_with_options(value, options).expect("optioned value emit");
    let mut written = Vec::new();
    saneyaml::to_writer_with_options(&mut written, value, options).expect("optioned value write");
    assert_eq!(written, emitted.as_bytes());

    let reparsed: Value = saneyaml::from_str(&emitted).expect("parse optioned emitted value");
    assert!(
        reparsed.equivalent(value),
        "optioned output changed value for {options:?}: {emitted}"
    );
    let emitted_again =
        saneyaml::to_string_with_options(&reparsed, options).expect("optioned re-emit value");
    assert_eq!(emitted_again, emitted);
}
