...
enum WorkflowOn {
    Event(String),
    Events(Vec<String>),
    Configured(BTreeMap<String, saneyaml::Value>),
}
...
fn rw_parse_github_actions__minimal_ci() {
    ...
    let workflow: Workflow = saneyaml::from_str(input).expect("deserialize workflow");
...
fn rw_parse_github_actions__matrix_ci() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize matrix workflow value");
    assert_eq!(
    ...
    let workflow: MatrixWorkflow = saneyaml::from_str(input).expect("deserialize matrix workflow");
...
fn rw_parse_github_actions__starter_node_ci_upstream_snapshot() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize starter workflow value");
    ...
    let workflow: StarterNodeWorkflow =
        saneyaml::from_str(input).expect("deserialize upstream starter workflow");
...
fn rw_parse_github_actions__polymorphic_workflow() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize polymorphic workflow value");
    ...
    let workflow: PolymorphicWorkflow =
        saneyaml::from_str(input).expect("deserialize polymorphic workflow");
    ...
    let runner_labels: RunnerLabels =
        saneyaml::from_str("[self-hosted, linux]\n").expect("runner label list");
    assert!(matches!(
        runner_labels,
...
struct AwesomeCompose {
    ...
    volumes: BTreeMap<String, Option<saneyaml::Value>>,
    secrets: BTreeMap<String, SecretFile>,
    networks: BTreeMap<String, Option<saneyaml::Value>>,
}
...
fn rw_parse_docker_compose__service_config() {
    ...
    let compose: Compose = saneyaml::from_str(input).expect("deserialize compose");
...
fn rw_parse_docker_compose__awesome_nginx_flask_mysql_upstream_snapshot() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize upstream compose value");
    assert_eq!(
    ...
    let compose: AwesomeCompose =
        saneyaml::from_str(input).expect("deserialize upstream compose typed config");
...
fn rw_parse_docker_compose__extension_anchors_and_default_merge_keys() {
    ...
    let compose: Compose = saneyaml::from_str(input).expect("deserialize compose with anchors");
    assert_eq!(compose.version, "3.9");
    assert_eq!(compose.services["web"].image, "nginx:latest");
    assert_eq!(
        compose.services["web"].ports.as_ref().unwrap(),
    ...
    let value: saneyaml::Value = saneyaml::from_str(input).expect("dynamic compose value");
    assert_eq!(
...
fn rw_parse_docker_compose__apply_merge_expands_extension_defaults() {
    let input = include_str!("fixtures/real-world/docker-compose/compose-anchors.yaml");
    let mut value: saneyaml::Value = saneyaml::from_str(input).expect("dynamic compose value");
...
fn rw_parse_docker_compose__platform_and_deploy_resources() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize compose resources value");
    assert_eq!(
...
fn rw_parse_docker_compose__polymorphic_service_fields() {
    ...
    let compose: PolymorphicCompose =
        saneyaml::from_str(input).expect("deserialize polymorphic compose");
    assert_eq!(compose.version, "3.9");
    assert!(matches!(
...
fn rw_parse_docker_compose__adapted_compose_spec_fragments_expand_merge_list() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize Compose spec fragments fixture");
    ...
    let stream =
        saneyaml::parse_lossless(input).expect("lossless graph for Compose spec fragments");
    let aliases = stream
        .aliases()
        .iter()
...
fn rw_parse_kubernetes__deployment_manifest() {
    ...
    let deployment: Deployment = saneyaml::from_str(input).expect("deserialize deployment");
...
struct CrdSchema {
    ...
    open_api_v3_schema: saneyaml::Value,
}
...
fn rw_parse_kubernetes__multi_doc_manifest_stream() {
    ...
    let docs: Vec<ManifestHeader> =
        saneyaml::from_documents_str(input).expect("deserialize manifest stream");
...
fn rw_parse_kubernetes__crd_openapi_schema_and_custom_resource_stream() {
    ...
    let docs: Vec<saneyaml::Value> =
        saneyaml::from_documents_str(input).expect("deserialize CRD/custom resource stream");

    assert_eq!(docs.len(), 2);
    assert_eq!(docs[0]["kind"].as_str(), Some("CustomResourceDefinition"));
    assert_eq!(docs[1]["kind"].as_str(), Some("Widget"));

    let crd: CustomResourceDefinition = saneyaml::from_value(docs[0].clone()).expect("typed CRD");
    ...
    let widget: Widget = saneyaml::from_value(docs[1].clone()).expect("typed custom resource");
...
fn rw_parse_kubernetes__multi_doc_values_preserve_resource_specs() {
    let input = include_str!("fixtures/real-world/kubernetes/multi-doc.yaml");
    let docs = saneyaml::Deserializer::from_str(input)
        .map(saneyaml::Value::deserialize)
...
fn rw_parse_kubernetes__helm_rendered_stream_preserves_empty_docs_and_ambiguous_strings() {
    ...
    let docs = saneyaml::from_documents_reader::<saneyaml::Value, _>(Cursor::new(input))
        .expect("stream values");
    let iter_docs = saneyaml::Deserializer::from_reader(Cursor::new(input))
        .map(saneyaml::Value::deserialize)
        .collect::<Result<Vec<_>, _>>()
...
fn rw_parse_kubernetes__configmap_block_scalars_preserve_data_comments() {
    ...
    let config: ConfigMap = saneyaml::from_str(input).expect("deserialize configmap");
...
fn rw_parse_helm__values_yaml() {
    ...
    let values: HelmValues =
        saneyaml::from_slice(input.as_bytes()).expect("deserialize helm values");
...
fn rw_parse_helm__chart_yaml_metadata_and_dependencies() {
    ...
    let chart: HelmChart = saneyaml::from_str(input).expect("deserialize helm chart");
    ...
    let value: saneyaml::Value = saneyaml::from_str(input).expect("deserialize helm chart value");
...
fn rw_parse_openapi__fragment() {
    ...
    let spec: OpenApi = saneyaml::from_reader(Cursor::new(input)).expect("deserialize openapi");
...
fn rw_parse_openapi__value_preserves_dynamic_schema_keys() {
    let input = include_str!("fixtures/real-world/openapi/petstore-fragment.yaml");
    let value: saneyaml::Value = saneyaml::from_str(input).expect("deserialize openapi value");
...
fn rw_parse_openapi__path_parameters_examples_and_extensions() {
    ...
    let docs = saneyaml::parse_documents(input).expect("parse rich openapi fixture");
    assert_eq!(docs.len(), 1);

    let value: saneyaml::Value = saneyaml::from_str(input).expect("deserialize rich openapi value");
    assert_eq!(
...
fn rw_parse_cloudflare__wrangler_yaml() {
    ...
    let wrangler: Wrangler = saneyaml::from_str(input).expect("deserialize wrangler yaml");
...
fn rw_parse_cloudflare__wrangler_direct_deserializer_borrows_from_input() {
    ...
    let wrangler: BorrowedWrangler<'_> =
        BorrowedWrangler::deserialize(saneyaml::Deserializer::from_str(input))
...
fn rw_parse_ansible__playbook() {
    ...
    let plays: Vec<Play> = saneyaml::from_str(input).expect("deserialize ansible playbook");
...
fn rw_parse_ansible__vault_and_unsafe_tags_preserve_value_tags_and_typed_reads() {
    ...
    let value: saneyaml::Value =
        saneyaml::from_str(input).expect("deserialize tagged ansible value");
    let vault = value[0]["vars"]["db_password"]
        .as_tagged()
        .expect("vault tag");
    assert_eq!(vault.tag, saneyaml::Tag::new("vault"));
    assert!(
        vault
            .value
            .as_str()
    ...
    let unsafe_template = value[0]["vars"]["raw_template"]
        .as_tagged()
        .expect("unsafe tag");
    assert_eq!(unsafe_template.tag, saneyaml::Tag::new("unsafe"));
    assert_eq!(
        unsafe_template.value.as_str(),
        Some("{{ literal_must_not_render }}")
    );

    let plays: Vec<TaggedPlay> =
        saneyaml::from_str(input).expect("deserialize tagged ansible play");
...
fn rw_parse_upstream_and_adapted_real_world_snapshots() {
    let kubernetes: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    let helm: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    let openapi: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    let wrangler: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    let ansible: saneyaml::Value = saneyaml::from_str(include_str!(
...
fn rw_parse_goal05_cloudformation_symfony_and_ci_fixtures() {
    let cloudformation: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    assert_eq!(table_ref.tag, saneyaml::Tag::new("Ref"));
    ...
    assert_eq!(inline_code.tag, saneyaml::Tag::new("Sub"));
    assert!(
        inline_code
            .value
            .as_str()
    ...
    assert_eq!(function_arn.tag, saneyaml::Tag::new("GetAtt"));
    assert_eq!(function_arn.value.as_str(), Some("Function.Arn"));

    let symfony: saneyaml::Value =
        saneyaml::from_str(include_str!("fixtures/real-world/symfony/services.yaml"))
    ...
    assert_eq!(tagged_iterator.tag, saneyaml::Tag::new("tagged_iterator"));
    ...
    let gitlab: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    let circle: saneyaml::Value =
        saneyaml::from_str(include_str!("fixtures/real-world/circleci/config.yml"))
    ...
    let azure: saneyaml::Value = saneyaml::from_str(include_str!(
    ...
    let github: saneyaml::Value = saneyaml::from_str(include_str!(
...
fn serde_type_errors_include_scalar_span() {
    let error = saneyaml::from_str::<BadReplicas>("replicas: many\n").expect_err("type error");
