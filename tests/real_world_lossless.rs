...
fn literal_scalar_values(stream: &saneyaml::LosslessStream) -> Vec<&str> {
    scalar_values(stream, ScalarStyle::Literal)
}

fn scalar_sources(stream: &saneyaml::LosslessStream, expected_style: ScalarStyle) -> Vec<&str> {
    stream
        .nodes()
        .iter()
        .filter_map(|node| match node.kind() {
...
fn scalar_values(stream: &saneyaml::LosslessStream, expected_style: ScalarStyle) -> Vec<&str> {
    stream
        .nodes()
        .iter()
        .filter_map(|node| match node.kind() {
...
fn collection_sources(
    stream: &saneyaml::LosslessStream,
    expected_style: CollectionStyle,
) -> Vec<&str> {
    stream
        .nodes()
        .iter()
...
fn assert_effective_scalar_entry(
    ...
    entries: &[saneyaml::LosslessEffectiveMappingEntry],
    key: &str,
    value: &str,
