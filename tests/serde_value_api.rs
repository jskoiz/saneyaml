...
struct SerializableRecursiveSingletonMapConfig {
    #[serde(with = "saneyaml::with::singleton_map_recursive")]
...
fn serde_api_from_slice_reader_and_value_roundtrip() {
    ...
    let from_slice: Config = saneyaml::from_slice(input).expect("from_slice");
    let from_reader: Config = saneyaml::from_reader(Cursor::new(input)).expect("from_reader");
    let from_reader_turbofish: Config =
        saneyaml::from_reader::<_, Config>(Cursor::new(input)).expect("from_reader generic order");
    ...
    let value: Value = saneyaml::from_slice(input).expect("deserialize into Value");
    ...
    let from_value: Config = saneyaml::from_value(value).expect("from_value");
...
fn serde_api_leading_utf8_bom_matches_serde_yaml_for_read_entrypoints() {
    ...
    let from_str: NameOnly = saneyaml::from_str(input).expect("from_str leading BOM");
    let from_slice: NameOnly =
        saneyaml::from_slice(input.as_bytes()).expect("from_slice leading BOM");
    let from_reader: NameOnly =
        saneyaml::from_reader(Cursor::new(input.as_bytes())).expect("from_reader leading BOM");
    let direct_str: NameOnly = NameOnly::deserialize(saneyaml::Deserializer::from_str(input))
        .expect("direct deserializer from_str leading BOM");
    let direct_slice: NameOnly =
        NameOnly::deserialize(saneyaml::Deserializer::from_slice(input.as_bytes()))
            .expect("direct deserializer from_slice leading BOM");
    let direct_reader: NameOnly = NameOnly::deserialize(saneyaml::Deserializer::from_reader(
    ...
    let map: BTreeMap<String, String> = saneyaml::from_str(input).expect("BOM mapping");
    ...
    let borrowed: BorrowedValue<'_> =
        saneyaml::from_str(borrowed_input).expect("borrowed BOM value");
    assert_eq!(borrowed.value, "app");
    assert_borrowed_from(borrowed_input, borrowed.value);

    let sequence_input = "\u{feff}- app\n";
    let sequence: Vec<String> = saneyaml::from_str(sequence_input).expect("BOM sequence");
    ...
    let flow: BTreeMap<String, String> = saneyaml::from_str(flow_input).expect("BOM flow mapping");
...
fn serde_api_deserialize_any_integer_dispatch_matches_serde_yaml_for_buffered_paths() {
    ...
    let ours: FlattenedYamlExtras = saneyaml::from_str(flatten_input).expect("yaml flatten extras");
    ...
    let tagged: TaggedService = saneyaml::from_str(tagged_input).expect("yaml tagged service");
    ...
    let node = saneyaml::parse_str(tagged_input).expect("parse tagged service");
    let from_node: TaggedService = saneyaml::from_node(&node).expect("from_node tagged service");
    ...
    let untagged: UntaggedNumericPort =
        saneyaml::from_str(untagged_input).expect("yaml untagged numeric port");
    ...
    let value: Value = saneyaml::from_str(untagged_input).expect("deserialize into Value");
    let from_value: UntaggedNumericPort =
        saneyaml::from_value(value.clone()).expect("from_value untagged numeric port");
...
fn serde_api_to_value_serializes_common_config_shapes_like_serde_yaml() {
    ...
    let from_value: Config = saneyaml::from_value(value).expect("deserialize to_value output");
    assert_eq!(
        from_value,
        Config {
            name: "app".to_string(),
...
fn serde_api_to_value_serializes_enum_shapes_like_serde_yaml() {
    ...
    for action in cases {
        let value = saneyaml::to_value(&action).expect("yaml enum to_value");
...
fn serde_api_to_value_preserves_yaml_value_tags() {
    ...
    let value = saneyaml::to_value(tagged).expect("serialize tagged yaml Value");
...
fn serde_api_to_value_keeps_ordinary_collect_str_tag_like_keys_as_mappings() {
    let value = saneyaml::to_value(SerializableCollectStrTagMap).expect("yaml collect_str tag map");
    let reference =
    ...
    let emitted =
        saneyaml::to_string(&SerializableCollectStrTagMap).expect("yaml collect_str tag output");
    let reparsed: Value = saneyaml::from_str(&emitted).expect("reparse collect_str tag output");
    assert_eq!(reparsed["!Thing"].as_str(), Some("x"));

    let ordinary = BTreeMap::from([("!Thing".to_string(), "x".to_string())]);
    let ordinary_value = saneyaml::to_value(&ordinary).expect("ordinary string key");
    assert!(ordinary_value.as_tagged().is_none());
    let ordinary_emitted =
        saneyaml::to_string(&ordinary).expect("yaml ordinary string tag-like key");
    let ordinary_reparsed: Value =
        saneyaml::from_str(&ordinary_emitted).expect("reparse ordinary string tag-like key");
...
fn serde_api_to_value_keeps_lone_bang_collect_str_keys_as_mappings_like_serde_yaml() {
    let value = saneyaml::to_value(SerializableCollectStrLoneBangMap)
        .expect("yaml collect_str lone bang map");
    ...
    assert_eq!(value["!"].as_str(), reference["!"].as_str());
    let emitted =
        saneyaml::to_string(&SerializableCollectStrLoneBangMap).expect("yaml lone bang output");
    let reparsed: Value = saneyaml::from_str(&emitted).expect("reparse lone bang output");
...
fn serde_api_tagged_value_serializes_like_serde_yaml_singleton_tag_map() {
    ...
    let value = saneyaml::to_value(&tagged).expect("yaml tagged value to_value");
    ...
    assert_eq!(
        saneyaml::to_string(&tagged).expect("yaml tagged value output"),
        serde_yaml::to_string(&reference).expect("serde_yaml tagged value output")
    );

    let external = saneyaml::to_value(&reference).expect("serde_yaml TaggedValue through yaml");
...
fn serde_api_non_specific_tagged_value_emits_and_reparses_with_string_semantics() {
    ...
    let value = saneyaml::to_value(&tagged).expect("yaml non-specific tagged value");
    ...
    let emitted = saneyaml::to_string(&tagged).expect("emit non-specific tag");
    let reparsed: Value = saneyaml::from_str(&emitted).expect("reparse non-specific tag");
...
where
    T: Copy + Serialize,
{
    let error = saneyaml::to_value(value).expect_err("yaml rejects empty variant tag");
    ...
    let mut serializer = saneyaml::Serializer::new(&mut writer);
...
fn serde_api_value_module_serializer_matches_to_value_path() {
    ...
    let helper = saneyaml::value::to_value(&config).expect("yaml value::to_value");
...
fn serde_api_value_byte_deserialization_matches_serde_yaml_value_policy() {
    ...
    assert!(
        saneyaml::from_value::<BytesFromDeserializeBytes>(value.clone()).is_err(),
    ...
    assert!(
        saneyaml::from_value::<BytesFromDeserializeByteBuf>(value.clone()).is_err(),
...
fn serde_api_parser_backed_byte_deserialization_rejects_like_serde_yaml() {
    ...
    let ours_vec: Vec<u8> = saneyaml::from_str("[0, 65, 255]\n").expect("yaml sequence to Vec<u8>");
    let reference_vec: Vec<u8> =
...
fn serde_api_explicit_binary_deserializes_to_byte_targets() {
    ...
    assert_eq!(
        saneyaml::from_str::<Vec<u8>>(input).expect("from_str explicit binary Vec<u8>"),
        expected
    );
    assert_eq!(
        saneyaml::from_slice::<Vec<u8>>(input.as_bytes())
            .expect("from_slice explicit binary Vec<u8>"),
        expected
    );
    assert_eq!(
        saneyaml::from_reader::<_, Vec<u8>>(Cursor::new(input.as_bytes()))
            .expect("from_reader explicit binary Vec<u8>"),
        expected
    );
    assert_eq!(
        BytesFromDeserializeBytes::deserialize(saneyaml::Deserializer::from_str(input))
            .expect("direct deserialize_bytes from str"),
        BytesFromDeserializeBytes(expected.clone())
    );
    assert_eq!(
        BytesFromDeserializeBytes::deserialize(saneyaml::Deserializer::from_slice(
            input.as_bytes()
        ))
    ...
    assert_eq!(
        BytesFromDeserializeBytes::deserialize(saneyaml::Deserializer::from_reader(Cursor::new(
            input.as_bytes()
        )))
    ...
    assert_eq!(
        BytesFromDeserializeByteBuf::deserialize(saneyaml::Deserializer::from_str(input))
            .expect("direct deserialize_byte_buf from str"),
        BytesFromDeserializeByteBuf(expected.clone())
    );
    assert_eq!(
        BytesFromDeserializeByteBuf::deserialize(saneyaml::Deserializer::from_slice(
            input.as_bytes()
        ))
    ...
    assert_eq!(
        BytesFromDeserializeByteBuf::deserialize(saneyaml::Deserializer::from_reader(Cursor::new(
            input.as_bytes()
        )))
    ...
    let node = saneyaml::parse_str(input).expect("explicit binary node");
    assert_eq!(
        saneyaml::from_node::<Vec<u8>>(&node).expect("node explicit binary Vec<u8>"),
        expected
    );
    assert_eq!(
    ...
    let value: Value = saneyaml::from_str(input).expect("explicit binary value");
    ...
    assert_eq!(
        saneyaml::from_value::<Vec<u8>>(value.clone()).expect("value explicit binary Vec<u8>"),
        expected
    );
    assert_eq!(
...
fn serde_api_invalid_explicit_binary_reports_scalar_span_for_byte_targets() {
    let input = "!!binary SGVsbG8?\n";
    let error = saneyaml::from_str::<Vec<u8>>(input).expect_err("invalid explicit binary");
    ...
    let value: Value =
        saneyaml::from_str(input).expect("invalid binary remains loadable as tagged value");
    let tagged = value.as_tagged().expect("retained binary tag");
    assert_eq!(tagged.tag, Tag::new("!!binary"));
    assert_eq!(tagged.value.as_str(), Some("SGVsbG8?"));
    assert!(
        saneyaml::from_value::<Vec<u8>>(value).is_err(),
        "typed byte targets validate retained !!binary payloads"
    );
}

fn assert_parser_backed_deserialize_bytes_rejected(input: &str) {
    let from_reader: saneyaml::Result<BytesFromDeserializeBytes> =
        saneyaml::from_reader(Cursor::new(input.as_bytes()));
    let node = saneyaml::parse_str(input).expect("parse byte rejection input");

    for (label, result) in [
        (
            "from_str",
            saneyaml::from_str::<BytesFromDeserializeBytes>(input),
        ),
        (
            "from_slice",
            saneyaml::from_slice::<BytesFromDeserializeBytes>(input.as_bytes()),
        ),
        ...
        (
            "direct from_reader",
            BytesFromDeserializeBytes::deserialize(saneyaml::Deserializer::from_reader(
                Cursor::new(input.as_bytes()),
            )),
        ),
        (
            "from_node",
            saneyaml::from_node::<BytesFromDeserializeBytes>(&node),
        ),
...
fn assert_parser_backed_deserialize_byte_buf_rejected(input: &str) {
    let from_reader: saneyaml::Result<BytesFromDeserializeByteBuf> =
        saneyaml::from_reader(Cursor::new(input.as_bytes()));
    let node = saneyaml::parse_str(input).expect("parse byte buffer rejection input");

    for (label, result) in [
        (
            "from_str",
            saneyaml::from_str::<BytesFromDeserializeByteBuf>(input),
        ),
        (
            "from_slice",
            saneyaml::from_slice::<BytesFromDeserializeByteBuf>(input.as_bytes()),
        ),
        ("from_reader", from_reader),
        (
            "direct from_str",
        ...
        (
            "direct from_reader",
            BytesFromDeserializeByteBuf::deserialize(saneyaml::Deserializer::from_reader(
                Cursor::new(input.as_bytes()),
            )),
        ),
        (
            "from_node",
            saneyaml::from_node::<BytesFromDeserializeByteBuf>(&node),
        ),
...
fn serde_api_to_value_bounds_hostile_sequence_and_tuple_length_hints() {
    for value in [
        saneyaml::to_value(HostileSequenceLengthHint).expect("hostile sequence hint"),
        saneyaml::to_value(HostileTupleLengthHint).expect("hostile tuple hint"),
        saneyaml::to_value(HostileTupleStructLengthHint).expect("hostile tuple struct hint"),
...
fn serde_api_to_string_bounds_hostile_sequence_length_hint() {
    let emitted = saneyaml::to_string(&HostileSequenceLengthHint).expect("hostile sequence output");
    let value: Value = saneyaml::from_str(&emitted).expect("parse hostile sequence output");
...
fn serde_api_writer_serializer_bounds_hostile_map_length_hint() {
    ...
    let value: Value = saneyaml::from_str(&emitted).expect("parse hostile map output");
...
fn serde_api_mapping_key_serializer_bounds_nested_key_length_hint() {
    let value = saneyaml::to_value(HostileMappingKeyLengthHint).expect("hostile mapping key hint");
...
fn serde_api_struct_and_variant_serializers_bound_length_hints() {
    let fields = saneyaml::to_value(HostileStructLengthHint).expect("hostile struct hint");
    assert_eq!(fields["k"].as_str(), Some("v"));

    let variant = saneyaml::to_value(HostileStructVariantLengthHint).expect("hostile variant hint");
...
fn serde_api_to_string_and_to_writer_serialize_common_config_shapes() {
    ...
    let emitted = saneyaml::to_string(&config).expect("yaml to_string");
    assert!(!emitted.starts_with("---\n"));
    let mut written = Vec::new();
    saneyaml::to_writer(&mut written, &config).expect("yaml to_writer");
    assert_eq!(String::from_utf8(written).expect("utf8 output"), emitted);

    let value: Value = saneyaml::from_str(&emitted).expect("parse emitted config");
    let reference = serde_yaml::to_value(&config).expect("serde_yaml to_value");
    assert_eq!(value["name"].as_str(), reference["name"].as_str());
    assert_eq!(value["ports"][0].as_u64(), reference["ports"][0].as_u64());
    assert_eq!(value["enabled"].as_bool(), reference["enabled"].as_bool());
...
fn serde_api_emit_options_structural_matches_default_writers() {
    ...
    let default = saneyaml::to_string(&config).expect("default emit");
    let structural = saneyaml::to_string_with_options(&config, saneyaml::EmitOptions::structural())
        .expect("structural emit");
    assert_eq!(structural, default);

    let mut written = Vec::new();
    saneyaml::to_writer_with_options(&mut written, &config, saneyaml::EmitOptions::structural())
    ...
    {
        let mut serializer =
            saneyaml::Serializer::with_options(&mut streamed, saneyaml::EmitOptions::structural());
...
fn serde_api_emit_options_byte_compatible_matches_serde_yaml_writer_corpus() {
        ...
        T: ?Sized + Serialize,
    {
        ...
        let emitted =
            saneyaml::to_string_with_options(value, saneyaml::EmitOptions::byte_compatible())
...
fn serde_api_emit_style_options_thread_through_all_writers() {
    ...
    let options = saneyaml::EmitOptions::structural()
        .with_key_order(saneyaml::KeyOrder::Sort)
        .with_scalar_quote_style(saneyaml::ScalarQuoteStyle::DoubleQuoted)
        .with_collection_style(saneyaml::EmitCollectionStyle::Flow);

    let emitted = saneyaml::to_string_with_options(&config, options).expect("optioned emit");
    assert_eq!(
        emitted,
        "{\"enabled\": true, \"env\": {\"RUST_LOG\": \"info\"}, \"name\": \"app\", \"optional\": null, \"ports\": [80, 443]}\n"
    );

    let mut written = Vec::new();
    saneyaml::to_writer_with_options(&mut written, &config, options).expect("optioned writer");
    assert_eq!(written, emitted.as_bytes());

    let mut streamed = Vec::new();
    {
        let mut serializer = saneyaml::Serializer::with_options(&mut streamed, options);
        config.serialize(&mut serializer).expect("optioned stream");
        serializer.flush().expect("flush optioned stream");
    }
    assert_eq!(streamed, emitted.as_bytes());

    let reparsed: Value = saneyaml::from_str(&emitted).expect("parse optioned output");
    let expected = saneyaml::to_value(&config).expect("expected value");
    assert!(reparsed.equivalent(&expected));
}
...
fn serde_api_document_writers_reject_bytes_like_serde_yaml() {
    ...
    let mut nested_serializer = saneyaml::Serializer::new(&mut nested_buffer);
...
fn serde_api_document_writers_serialize_top_level_values_once() {
    ...
    let emitted = saneyaml::to_string(&value).expect("yaml to_string");
    ...
    let mut written = Vec::new();
    saneyaml::to_writer(&mut written, &writer_value).expect("yaml to_writer");
...
fn serde_api_document_writers_serialize_nested_map_entries_once() {
    ...
    let emitted = saneyaml::to_string(&value).expect("yaml map");
...
fn serde_api_collect_str_mapping_keys_format_display_once() {
    ...
    let value = saneyaml::to_value(OneShotCollectStrMap {
    ...
    let emitted = saneyaml::to_string(&OneShotCollectStrMap {
...
fn serde_api_document_writers_serialize_struct_fields_once() {
    ...
    let emitted = saneyaml::to_string(&value).expect("yaml struct");
    assert_eq!(value_calls.get(), 1);
...
fn serde_api_document_writers_serialize_enum_variant_payloads_once() {
    ...
    let emitted_newtype = saneyaml::to_string(&newtype).expect("yaml newtype variant");
    ...
    let emitted_tuple = saneyaml::to_string(&tuple).expect("yaml tuple variant");
    ...
    let emitted_struct = saneyaml::to_string(&struct_variant).expect("yaml struct variant");
...
fn serde_api_writer_serializer_streams_multiple_documents() {
    ...
    let docs: Vec<Value> = saneyaml::from_documents_str(&output).expect("parse serialized stream");
...
fn serde_api_writer_serializer_into_inner_returns_writer() {
    ...
    let mut serializer = saneyaml::Serializer::new(Cursor::new(Vec::new()));
    config
        .serialize(&mut serializer)
    ...
    let value: Value = saneyaml::from_str(&output).expect("parse serialized document");
...
fn serde_api_with_singleton_map_serializes_enum_fields_like_serde_yaml() {
    ...
    let value = saneyaml::to_value(&config).expect("yaml singleton serialize");
    let reference =
    ...
    let emitted = saneyaml::to_string(&config).expect("emit singleton map");
    let reparsed: Value = saneyaml::from_str(&emitted).expect("parse singleton map");
    assert_eq!(
...
fn serde_api_with_singleton_map_recursive_serializes_nested_enum_fields_like_serde_yaml() {
    ...
    let value = saneyaml::to_value(&config).expect("yaml recursive singleton serialize");
    let reference =
...
";
    ...
    let parsed: SingletonMapConfig = saneyaml::from_str(input).expect("yaml singleton_map helper");
...
";
    ...
    let plain: PlainTaggedConfig = saneyaml::from_str(input).expect("plain tagged enum shape");
    assert_eq!(
        plain.action,
        SingletonAction::Shell {
            run: "cargo test".to_string()
    ...
    let yaml_error =
        saneyaml::from_str::<SingletonMapConfig>(input).expect_err("tagged helper input rejected");
...
";
    ...
    let yaml_error =
        saneyaml::from_str::<SingletonMapConfig>(input).expect_err("multi-key variant rejected");
    let reference_error = serde_yaml::from_str::<ReferenceSingletonMapConfig>(input)
...
";
    ...
    let parsed: RecursiveSingletonMapConfig =
        saneyaml::from_str(input).expect("yaml recursive singleton_map helper");
    ...
    let direct: RecursiveActions = saneyaml::with::singleton_map_recursive::deserialize(
        saneyaml::Deserializer::from_str(&input["actions:\n".len()..]),
    )
...
";
    ...
    let yaml_error = saneyaml::from_str::<RecursiveSingletonMapConfig>(input)
...
fn serde_api_typed_config_reads_alias_expanded_values() {
    ...
    let compose: AliasCompose = saneyaml::from_str(input).expect("deserialize alias compose");
...
fn serde_api_value_collections_are_spanless() {
    let value: Value = saneyaml::from_str("name: app\nports: [80, 443]\n").expect("value");
...
fn serde_api_value_mutation_and_defaults_match_read_side_expectations() {
    let mut value: Value = saneyaml::from_str("items: [one]\n").expect("value");
    value
        .get_mut("items")
...
fn serde_api_value_index_mut_matches_serde_yaml_for_config_patching() {
    let mut value: Value = saneyaml::from_str("{x: 0}\n").expect("value");
...
fn serde_api_mapping_entry_matches_serde_yaml_normalization_helpers() {
    ...
    *reference
    ...
    match mapping.entry(Value::String("image".to_string())) {
        ...
        saneyaml::mapping::Entry::Vacant(_) => panic!("image should be occupied"),
    }
...
fn serde_api_value_usize_index_reads_numeric_mapping_keys() {
    let input = "0: zero\n42: answer\nitems: [first]\n";
    let mut value: Value = saneyaml::from_str(input).expect("value");
    ...
    let mut mapping: Mapping = saneyaml::from_str("42: answer\n").expect("mapping");
...
fn serde_api_mapping_matches_common_serde_yaml_read_helpers() {
    let input = "a: 1\nb: two\nc: true\n";
    let mut mapping: Mapping = saneyaml::from_str(input).expect("deserialize saneyaml::Mapping");
...
fn serde_api_mapping_iterators_match_public_serde_yaml_surface() {
    fn assert_value_index<T: ?Sized + saneyaml::Index>() {}
    fn assert_mapping_index<T: ?Sized + saneyaml::mapping::Index>() {}
    ...
    let input = "a: 1\nb: two\nc: true\n";
    let mut mapping: Mapping = saneyaml::from_str(input).expect("deserialize mapping");

    let iter: saneyaml::mapping::Iter<'_> = mapping.iter();
    ...
    let keys: saneyaml::mapping::Keys<'_> = mapping.keys();
    ...
    let values: saneyaml::mapping::Values<'_> = mapping.values();
    assert_eq!(values.len(), 3);
    assert_eq!(values.filter(|value| value.is_number()).count(), 1);

    {
        let by_ref: saneyaml::mapping::Iter<'_> = (&mapping).into_iter();
        assert_eq!(by_ref.len(), 3);
    }

    {
        let mut iter_mut: saneyaml::mapping::IterMut<'_> = mapping.iter_mut();
    ...
    {
        let mut values_mut: saneyaml::mapping::ValuesMut<'_> = mapping.values_mut();
    ...
    {
        let by_mut: saneyaml::mapping::IterMut<'_> = (&mut mapping).into_iter();
        assert_eq!(by_mut.len(), 3);
    }

    let into_iter: saneyaml::mapping::IntoIter = mapping.clone().into_iter();
    ...
    let mut into_keys: saneyaml::mapping::IntoKeys = mapping.clone().into_keys();
    ...
    let mut into_values: saneyaml::mapping::IntoValues = mapping.into_values();
...
fn serde_api_value_null_deserializes_as_empty_collections_like_serde_yaml() {
    ...
    let ours_vec: Vec<String> = saneyaml::from_value(Value::Null).expect("null to vec");
    assert_eq!(ours_vec, reference_vec);

    let reference_map: BTreeMap<String, String> =
        serde_yaml::from_value(serde_yaml::Value::Null).expect("serde_yaml null to map");
    let ours_map: BTreeMap<String, String> =
        saneyaml::from_value(Value::Null).expect("null to map");
    ...
    let ours_struct: DefaultedCollections =
        saneyaml::from_value(Value::Null).expect("null to defaulted struct");
    ...
    let direct_null = saneyaml::from_str::<Vec<String>>("null\n")
...
fn serde_api_parser_backed_empty_nodes_deserialize_as_empty_collections_like_serde_yaml() {
    for input in ["", "---\n"] {
        ...
        let ours_vec: Vec<String> = saneyaml::from_str(input).expect("from_str vec");
        let ours_slice: Vec<String> =
            saneyaml::from_slice(input.as_bytes()).expect("from_slice vec");
        let node = saneyaml::parse_str(input).expect("node");
        let ours_node: Vec<String> = saneyaml::from_node(&node).expect("from_node vec");
        let ours_document: Vec<String> =
            Vec::deserialize(saneyaml::Deserializer::from_str(input)).expect("document vec");
        assert_eq!(ours_vec, reference_vec);
        ...
        let ours_map: BTreeMap<String, String> = saneyaml::from_str(input).expect("from_str map");
        let ours_node_map: BTreeMap<String, String> =
            saneyaml::from_node(&node).expect("from_node map");
        assert_eq!(ours_map, reference_map);
        ...
        let ours_struct: DefaultedCollections = saneyaml::from_str(input).expect("from_str struct");
        let ours_node_struct: DefaultedCollections =
            saneyaml::from_node(&node).expect("from_node struct");
        assert_eq!(ours_struct, reference_struct);
    ...
    let ours: DefaultedCollections =
        saneyaml::from_str(empty_fields).expect("empty field collections");
    assert_eq!(ours, reference);

    let stream = "---\n---\nports: [80]\nlabels:\n  env: prod\n";
    let ours_docs = saneyaml::Deserializer::from_str(stream)
    ...
    for explicit_null in ["null\n", "~\n"] {
        let error = saneyaml::from_str::<Vec<String>>(explicit_null)
            .expect_err("explicit null is not a parser-backed sequence");
        assert!(error.to_string().contains("expected"));
    }
    let explicit_null_field = saneyaml::from_str::<DefaultedCollections>("ports: null\nlabels:\n")
...
fn serde_api_explicit_empty_documents_are_null() {
    let input = "---\n---\nname: second\n";
    let docs: Vec<Value> = saneyaml::from_documents_str(input).expect("docs");
    assert_eq!(docs.len(), 2);
    assert!(docs[0].is_null());
    assert_eq!(docs[1]["name"].as_str(), Some("second"));

    let values = saneyaml::Deserializer::from_slice(input.as_bytes())
...
fn serde_api_empty_stream_iterator_matches_serde_yaml_null_document() {
    let values = saneyaml::Deserializer::from_str("")
        .map(Value::deserialize)
        .collect::<Result<Vec<_>, _>>()
    ...
    let from_slice = saneyaml::Deserializer::from_slice(b"")
        .map(Value::deserialize)
        .collect::<Result<Vec<_>, _>>()
        .expect("yaml empty slice stream values");
    assert_eq!(from_slice, values);

    let from_reader = saneyaml::Deserializer::from_reader(Cursor::new(Vec::<u8>::new()))
        .map(Value::deserialize)
        .collect::<Result<Vec<_>, _>>()
...
fn serde_api_deserializer_iterates_document_stream() {
    let input = "---\nname: one\nports: [80]\nenabled: true\n---\nname: two\nports: [443]\nenabled: false\n";
    let configs = saneyaml::Deserializer::from_str(input)
    ...
    let values = saneyaml::Deserializer::from_reader(Cursor::new(input))
        .map(Value::deserialize)
        .collect::<Result<Vec<_>, _>>()
        .expect("stream values");
...
fn serde_api_deserializer_stream_items_match_public_serde_yaml_surface() {
    ...
    where
        I: Iterator<Item = saneyaml::Deserializer<'de>>,
    {
        iter
    }
    ...
    let input = "---\nname: one\n---\nname: two\n";
    let mut ours = assert_yaml_stream_items(saneyaml::Deserializer::from_str(input));
...
fn serde_api_direct_deserializer_errors_match_public_yaml_error_surface() {
    fn assert_yaml_error_result<T>(result: Result<T, saneyaml::Error>) -> T {
        result.expect("deserialization through public saneyaml::Error")
    }

    let direct: Value = assert_yaml_error_result(Value::deserialize(
        saneyaml::Deserializer::from_str("name: direct\n"),
    ));
    assert_eq!(direct["name"].as_str(), Some("direct"));

    let mut stream = saneyaml::Deserializer::from_str("---\nname: stream\n");
    ...
    let node = saneyaml::parse_str("name: node\n").expect("parse node");
...
fn serde_api_stream_yields_prior_value_doc_before_later_parse_error() {
    let input =
        "---\ndefaults: &defaults\n  port: !Port 8080\nservice: *defaults\n---\nbad: *missing\n";
    let mut docs = saneyaml::Deserializer::from_str(input);
...
fn serde_api_direct_deserializer_preserves_related_parse_diagnostics() {
    ...
    let from_str_error = saneyaml::from_str::<Value>(input).expect_err("duplicate key");
    let direct_error = Value::deserialize(saneyaml::Deserializer::from_str(input))
...
fn serde_api_ignored_any_direct_deserializer_validates_before_skipping() {
    IgnoredAny::deserialize(saneyaml::Deserializer::from_str("name: api\n"))
        .expect("valid single document can be ignored");

    let malformed = "[ok]\ntrailing: bad\n";
    let error = IgnoredAny::deserialize(saneyaml::Deserializer::from_str(malformed))
    ...
    let multi_doc = "---\nname: api\n---\nname: worker\n";
    let error = IgnoredAny::deserialize(saneyaml::Deserializer::from_str(multi_doc))
...
fn serde_api_flow_mapping_metadata_sequence_keys_deserialize_to_vec_keys() {
    let anchored: FlowSequenceKeyRoot =
        saneyaml::from_str("root: {? &seq [a, b] : seq-v}\n").expect("anchored sequence key");
    assert_eq!(
    ...
    let tagged: FlowSequenceKeyRoot =
        saneyaml::from_str("root: {? !Thing [c, d] : tagged-v}\n").expect("tagged sequence key");
    assert_eq!(
...
fn serde_api_flow_mapping_complex_key_rejects_string_key_with_key_span() {
    let error = saneyaml::from_str::<FlowStringKeyRoot>("root: {? {a: b}: value}\n")
...
fn serde_api_deserializer_can_drive_single_document_deserialize() {
    let input = "name: app\nports: [80]\nenabled: true\n";
    let config =
        Config::deserialize(saneyaml::Deserializer::from_str(input)).expect("deserialize config");
    assert_eq!(
        config,
        Config {
            name: "app".to_string(),
    ...
    let value = Value::deserialize(saneyaml::Deserializer::from_slice(input.as_bytes()))
        .expect("deserialize value");
    assert_eq!(value["name"].as_str(), Some("app"));

    let port = u16::deserialize(saneyaml::Deserializer::from_str("!Port 8080"))
        .expect("tagged scalar through deserializer");
    assert_eq!(port, 8080);
    let optional = Option::<String>::deserialize(saneyaml::Deserializer::from_str("!Maybe null"))
...
fn serde_api_single_document_deserializer_rejects_streams() {
    let input = "---\nname: one\n---\nname: two\n";
    let error = Value::deserialize(saneyaml::Deserializer::from_str(input))
...
";
    ...
    let values: Vec<TaggedEnum> = saneyaml::from_str(input).expect("tagged enum values");
    assert_eq!(
        values,
        vec![
            TaggedEnum::Unit,
...
fn serde_api_value_preserves_yaml_tags() {
    let value: Value = saneyaml::from_str(
...
fn serde_api_tagged_value_deserializes_directly_like_serde_yaml() {
    ...
    let values: BTreeMap<String, TaggedValue> =
        saneyaml::from_str(input).expect("direct tagged values");
    let node = saneyaml::parse_str(input).expect("tagged value node");
    let from_node: BTreeMap<String, TaggedValue> =
        saneyaml::from_node(&node).expect("direct tagged values from node");
    let value: Value = saneyaml::from_str(input).expect("tagged Value");
    let from_value: BTreeMap<String, TaggedValue> =
        saneyaml::from_value(value.clone()).expect("direct tagged values from value");
    let from_value_ref =
    ...
    let error = saneyaml::from_str::<TaggedValue>("plain\n").expect_err("untagged value rejected");
    assert!(
...
fn serde_api_value_preserves_core_local_and_verbatim_tags() {
    ...
    let node = saneyaml::parse_bytes(fuzz_input).expect("parse fuzz regression");
    let from_node = Value::from(&node);
    let from_slice: Value = saneyaml::from_slice(fuzz_input).expect("deserialize fuzz regression");

    assert!(from_node.equivalent(&from_slice));

    let leading_bang = saneyaml::parse_str("!<!ab> value\n").expect("parse verbatim bang tag");
    let direct = Value::from(&leading_bang);
    let from_node: Value =
        saneyaml::from_node(&leading_bang).expect("deserialize verbatim bang tag");
...
nan: !!float .nan
";
    let ours: ExplicitCoreNumbers = saneyaml::from_str(input).expect("explicit core numeric tags");
    assert_explicit_core_numbers(&ours);

    let node = saneyaml::parse_str(input).expect("node");
    let from_node: ExplicitCoreNumbers =
        saneyaml::from_node(&node).expect("explicit core numeric tags from node");
    ...
    let from_value: ExplicitCoreNumbers =
        saneyaml::from_value(value.clone()).expect("explicit core numeric tags from value");
    let from_value_ref =
...
fn serde_api_canonical_core_tags_match_short_core_tag_semantics() {
    ...
    struct CanonicalCoreTags {
        ...
        canonical_timestamp: saneyaml::Timestamp,
    }
...
";
    let expected = CanonicalCoreTags {
        ...
        canonical_timestamp: saneyaml::Timestamp::parse_yaml_1_1("2026-05-25").expect("timestamp"),
    };

    let typed: CanonicalCoreTags = saneyaml::from_str(input).expect("canonical core tags");
    let direct = CanonicalCoreTags::deserialize(saneyaml::Deserializer::from_str(input))
        .expect("direct canonical core tags");
    let node = saneyaml::parse_str(input).expect("canonical core tag node");
    let from_node: CanonicalCoreTags =
        saneyaml::from_node(&node).expect("canonical core tags from node");
    let value: Value = saneyaml::from_str(input).expect("canonical core tag value");
    let from_value: CanonicalCoreTags =
        saneyaml::from_value(value.clone()).expect("canonical core tags from value");
    let from_value_ref =
    ...
    assert_eq!(
        ...
        saneyaml::Timestamp::parse_yaml_1_1("2026-05-25")
    );
}
...
";
    ...
    let typed: CollectionTags = saneyaml::from_str(input).expect("YAML 1.1 collection tags");
    let direct = CollectionTags::deserialize(saneyaml::Deserializer::from_str(input))
        .expect("direct YAML 1.1 collection tags");
    let node = saneyaml::parse_str(input).expect("YAML 1.1 collection tag node");
    let from_node: CollectionTags =
        saneyaml::from_node(&node).expect("YAML 1.1 collection tags from node");
    let value: Value = saneyaml::from_str(input).expect("YAML 1.1 collection tag value");
    let from_value: CollectionTags =
        saneyaml::from_value(value.clone()).expect("YAML 1.1 collection tags from value");
    let from_value_ref =
...
";
    ...
    let typed: ExplicitCoreScalars = saneyaml::from_str(input).expect("explicit core scalar tags");
    let direct: ExplicitCoreScalars =
        ExplicitCoreScalars::deserialize(saneyaml::Deserializer::from_str(input))
            .expect("direct explicit core scalar tags");
    let node = saneyaml::parse_str(input).expect("explicit core scalar tag node");
    let from_node: ExplicitCoreScalars =
        saneyaml::from_node(&node).expect("explicit core scalar tags from node");
    let value: Value = saneyaml::from_str(input).expect("explicit core scalar tag value");
    let from_value: ExplicitCoreScalars =
        saneyaml::from_value(value.clone()).expect("explicit core scalar tags from value");
    let from_value_ref =
    ...
    let bool_error =
        saneyaml::from_str::<bool>("!!bool maybe\n").expect_err("invalid explicit bool");
    assert!(
        bool_error
            .to_string()
    ...
    let null_error =
        saneyaml::from_str::<Option<String>>("!!null foo\n").expect_err("invalid explicit null");
    assert!(
        null_error
            .to_string()
...
fn serde_api_percent_tag_resolves_handles_and_stays_transparent_for_typed_reads() {
    ...
    let value: Value = saneyaml::from_str(input).expect("tag directive value");
    ...
    let typed: BTreeMap<String, String> = saneyaml::from_str(input).expect("typed transparent tag");
...
fn serde_api_tag_directives_require_explicit_document_start_and_do_not_leak() {
    ...
    let error = saneyaml::from_str::<Value>(missing_start).expect_err("directive without marker");
    assert!(error.to_string().contains("explicit document start"));

    let declared = "%TAG !e! tag:example.com,2026:\n---\n!e!Thing one\n";
    let value: Value = saneyaml::from_str(declared).expect("declared tag");
    let tagged = value.as_tagged().expect("resolved tag");
    assert_eq!(tagged.tag.suffix, "tag:example.com,2026:Thing");

    let scoped = "%TAG !e! tag:example.com,2026:\n---\n!e!Thing one\n...\n---\n!e!Thing two\n";
    let error =
        saneyaml::from_documents_str::<Value>(scoped).expect_err("tag directive does not leak");
    assert!(
        error
            .to_string()
...
fn serde_api_tagged_value_can_drive_enum_deserialize_by_value_and_reference() {
    ...
    let owned: TaggedEnum = saneyaml::from_value(value.clone()).expect("tagged enum by value");
...
";
    ...
    let from_str: TaggedConfig = saneyaml::from_str(input).expect("from_str");
    let from_slice: TaggedConfig = saneyaml::from_slice(input.as_bytes()).expect("from_slice");
    let from_reader: TaggedConfig =
        saneyaml::from_reader(Cursor::new(input.as_bytes())).expect("from_reader");
    let node = saneyaml::parse_str(input).expect("parse node");
    let from_node: TaggedConfig = saneyaml::from_node(&node).expect("from_node");
    let value: Value = saneyaml::from_str(input).expect("value");
    let from_value: TaggedConfig = saneyaml::from_value(value.clone()).expect("from_value");
    let from_value_ref =
...
fn serde_api_tagged_anchor_alias_matrix_preserves_tags_across_entrypoints() {
    ...
    for case in cases {
        ...
        let node = saneyaml::parse_str(case.input)
        ...
        let from_node: Value = saneyaml::from_node(&node)
        ...
        let from_reader: Value = saneyaml::from_reader(Cursor::new(case.input.as_bytes()))
        ...
        let from_value: Value = saneyaml::from_value(from_str.clone())
        ...
        let document = saneyaml::Deserializer::from_str(case.input)
            .next()
        ...
        let document = saneyaml::Deserializer::from_slice(case.input.as_bytes())
            .next()
        ...
        let document = saneyaml::Deserializer::from_reader(Cursor::new(case.input.as_bytes()))
            .next()
        ...
        for (surface, docs) in [
            (
                "from_documents_str",
                saneyaml::from_documents_str::<Value>(case.input),
            ),
            (
                "from_documents_slice",
                saneyaml::from_documents_slice::<Value>(case.input.as_bytes()),
            ),
            (
                "from_documents_reader",
                saneyaml::from_documents_reader::<Value, _>(Cursor::new(case.input.as_bytes())),
            ),
        ] {
...
{
    let from_str =
        saneyaml::from_str::<T>(input).unwrap_or_else(|error| panic!("{name} from_str: {error}"));
    assert_eq!(from_str, expected, "{name} from_str");

    let from_slice = saneyaml::from_slice::<T>(input.as_bytes())
        .unwrap_or_else(|error| panic!("{name} from_slice: {error}"));
    assert_eq!(from_slice, expected, "{name} from_slice");

    let from_reader = saneyaml::from_reader::<_, T>(Cursor::new(input.as_bytes()))
        .unwrap_or_else(|error| panic!("{name} from_reader: {error}"));
    assert_eq!(from_reader, expected, "{name} from_reader");

    let node =
        saneyaml::parse_str(input).unwrap_or_else(|error| panic!("{name} parse node: {error}"));
    let from_node =
        saneyaml::from_node::<T>(&node).unwrap_or_else(|error| panic!("{name} from_node: {error}"));
    assert_eq!(from_node, expected, "{name} from_node");

    let value =
        saneyaml::from_str::<Value>(input).unwrap_or_else(|error| panic!("{name} value: {error}"));
    let from_value = saneyaml::from_value::<T>(value.clone())
    ...
    let document = saneyaml::Deserializer::from_str(input)
        .next()
    ...
    let document = saneyaml::Deserializer::from_slice(input.as_bytes())
        .next()
    ...
    let document = saneyaml::Deserializer::from_reader(Cursor::new(input.as_bytes()))
        .next()
    ...
    for (surface, docs) in [
        (
            "from_documents_str",
            saneyaml::from_documents_str::<T>(input),
        ),
        (
            "from_documents_slice",
            saneyaml::from_documents_slice::<T>(input.as_bytes()),
        ),
        (
            "from_documents_reader",
            saneyaml::from_documents_reader::<T, _>(Cursor::new(input.as_bytes())),
        ),
    ] {
...
fn serde_api_tags_are_transparent_for_borrowed_retained_reads() {
    ...
    let node = saneyaml::parse_str(input).expect("parse node");
    let from_node: TaggedBorrowedConfig<'_> = saneyaml::from_node(&node).expect("from_node");
    let document = saneyaml::Deserializer::from_str(input)
        .next()
        .expect("document");
...
fn serde_api_verbatim_tags_preserve_suffix_and_are_transparent() {
    ...
    let typed: BTreeMap<String, String> = saneyaml::from_str(input).expect("typed map");
    assert_eq!(typed["value"], "tagged");
}
...
fn serde_api_yaml11_timestamps_have_native_typed_reads_and_string_transparency() {
    ...
    struct TimestampScalars {
        date: saneyaml::Timestamp,
        datetime: saneyaml::Timestamp,
        explicit: saneyaml::Timestamp,
    }
...
";
    ...
    assert_eq!(
        ...
        saneyaml::Timestamp::parse_yaml_1_1("2026-05-24")
    );
    assert_eq!(
        value["datetime"].as_timestamp(),
        saneyaml::Timestamp::parse_yaml_1_1("2026-05-24T12:34:56Z")
    );
    assert_eq!(
    ...
    let from_value: TaggedScalars =
        saneyaml::from_value(value.clone()).expect("owned value typed strings");
    ...
    let expected_timestamps = TimestampScalars {
        date: saneyaml::Timestamp::parse_yaml_1_1("2026-05-24").expect("date timestamp"),
        datetime: saneyaml::Timestamp::parse_yaml_1_1("2026-05-24T12:34:56Z")
            .expect("datetime timestamp"),
        explicit: saneyaml::Timestamp::parse_yaml_1_1("2026-05-25").expect("explicit timestamp"),
    };
    ...
    let from_value_timestamps: TimestampScalars =
        saneyaml::from_value(value.clone()).expect("owned value typed timestamps");
...
fn serde_api_string_targets_match_serde_yaml_null_like_scalars() {
    ...
    let parsed: BTreeMap<String, String> = saneyaml::from_str(input).expect("typed string map");
    ...
    let optional: BTreeMap<String, Option<String>> =
        saneyaml::from_str(input).expect("typed option string map");
    ...
    let value: Value = saneyaml::from_str(input).expect("value map");
...
fn serde_api_string_targets_preserve_plain_scalar_source_for_typed_reads() {
    ...
    let parsed: SourceBackedStringTargets =
        saneyaml::from_str(input).expect("typed string targets");
    let parsed_from_slice: SourceBackedStringTargets =
        saneyaml::from_slice(input.as_bytes()).expect("typed string targets from slice");
    let parsed_direct_slice: SourceBackedStringTargets = SourceBackedStringTargets::deserialize(
    ...
    let value: Value = saneyaml::from_str(input).expect("value map");
    let error = saneyaml::from_value::<SourceBackedStringTargets>(value)
...
fn serde_api_stream_document_deserializer_preserves_type_error_span() {
    let input = "---\nname: app\nports: no\nenabled: true\n";
    let error = saneyaml::Deserializer::from_str(input)
        .map(Config::deserialize)
        .next()
        .expect("one document")
...
fn serde_api_numeric_range_errors_preserve_scalar_span() {
    ...
    let direct_error = saneyaml::from_str::<u16>("70000\n").expect_err("top-level u16 range error");
...
fn serde_api_from_node_supports_borrowed_config_fields() {
    let node = saneyaml::parse_str("name: node\npath: /srv/node\n").expect("node");
    let config: BorrowedConfig<'_> = saneyaml::from_node(&node).expect("borrowed config");
...
fn serde_api_from_str_and_from_slice_support_borrowed_config_fields() {
    ...
    let from_str: BorrowedConfig<'_> = saneyaml::from_str(input).expect("borrowed from_str");
    let from_slice: BorrowedConfig<'_> =
        saneyaml::from_slice(input.as_bytes()).expect("borrowed from_slice");
...
fn serde_api_deserializer_from_str_and_from_slice_borrow_from_source() {
    ...
    let from_str: BorrowedConfig<'_> =
        BorrowedConfig::deserialize(saneyaml::Deserializer::from_str(input))
            .expect("borrowed direct deserializer from_str");
    let from_slice: BorrowedConfig<'_> =
        BorrowedConfig::deserialize(saneyaml::Deserializer::from_slice(input.as_bytes()))
...
fn serde_api_deserializer_borrows_source_backed_string_targets() {
    ...
    let parsed: BorrowedScalarTargets<'_> =
        BorrowedScalarTargets::deserialize(saneyaml::Deserializer::from_str(input))
            .expect("borrowed source-backed direct deserializer");
    let parsed_from_slice: BorrowedScalarTargets<'_> =
        BorrowedScalarTargets::deserialize(saneyaml::Deserializer::from_slice(input.as_bytes()))
...
fn serde_api_deserializer_reader_remains_owned_for_borrowed_targets() {
    let input = "name: app\npath: /srv/app\n";
    let error = BorrowedConfig::deserialize(saneyaml::Deserializer::from_reader(Cursor::new(
        input.as_bytes(),
    )))
...
fn serde_api_from_str_borrows_plain_scalar_map_keys_and_source_backed_values() {
    let input = include_str!("fixtures/divergences/source-backed-string-targets.yaml");
    let parsed: BorrowedScalarTargets<'_> =
        saneyaml::from_str(input).expect("borrowed source-backed string targets");
    let parsed_from_slice: BorrowedScalarTargets<'_> = saneyaml::from_slice(input.as_bytes())
...
fn serde_api_borrowed_entrypoints_match_serde_yaml_for_transformed_scalars() {
    for (name, input) in [
        ...
        let ours =
            saneyaml::from_str::<BorrowedValue<'_>>(input).map(|value| value.value.to_string());
        let reference =
        ...
        let ours_cow: CowValue<'_> = saneyaml::from_str(input).expect("Cow value");
...
fn serde_api_borrowed_entrypoint_errors_preserve_spans() {
    let unknown = saneyaml::from_str::<StrictConfig>("name: app\nextra: true\n")
    ...
    let direct_range =
        StreamPortConfig::deserialize(saneyaml::Deserializer::from_str("port: 70000\n"))
    ...
    invalid.push(0xFF);
    let utf8 = saneyaml::from_slice::<BorrowedConfig<'_>>(&invalid).expect_err("invalid UTF-8");
    ...
    let direct_utf8 = BorrowedConfig::deserialize(saneyaml::Deserializer::from_slice(&invalid))
...
fn serde_api_value_reference_deserializer_borrows_strings() {
    let value: Value = saneyaml::from_str("name: value\npath: /srv/value\n").expect("value");
...
fn serde_api_unknown_field_error_uses_key_span() {
    let node = saneyaml::parse_str("name: app\nextra: true\n").expect("node");
    let error = saneyaml::from_node::<StrictConfig>(&node).expect_err("unknown field");
...
";
    ...
    let mut value: Value = saneyaml::from_str(input).expect("value with default merge");
...
fn serde_api_caller_built_value_deserializers_expand_merge_keys_by_default() {
    ...
    let from_value: CallerBuiltMergeRoot =
        saneyaml::from_value(value.clone()).expect("from_value expands caller-built merge");
    ...
    let normalized: Value = saneyaml::from_value(value.clone()).expect("from_value to Value");
...
fn serde_api_caller_built_value_enum_deserializers_expand_merge_keys_by_default() {
    ...
    let from_value: SingletonAction =
        saneyaml::from_value(value.clone()).expect("from_value expands enum merge");
...
fn serde_api_nested_caller_built_value_enum_merges_expand_by_default() {
    ...
    let from_value: CallerBuiltMergeActionRoot =
        saneyaml::from_value(value.clone()).expect("from_value expands nested enum merge");
...
fn serde_api_caller_built_value_merge_respects_literal_tagged_keys() {
    ...
    let normalized: Value =
        saneyaml::from_value(Value::Mapping(target)).expect("caller-built tagged merge");
    ...
    assert!(
        keys.iter().all(|key| !matches!(key, Value::Tagged(tagged)
            if tagged.tag == saneyaml::Tag::new("!!merge")
    ...
    assert!(
        keys.iter().any(|key| matches!(key, Value::Tagged(tagged)
            if tagged.tag == saneyaml::Tag::new("!Thing")
                && tagged.value.as_str() == Some("<<"))),
...
fn serde_api_caller_built_value_merge_reports_invalid_payloads() {
    ...
    let error = saneyaml::from_value::<Value>(Value::Mapping(target.clone()))
...
";
    ...
    let mut value: Value = saneyaml::from_str(input).expect("value with merge list");
    let mut reference: serde_yaml::Value =
...
";
    ...
    let mut value: Value = saneyaml::from_str(input).expect("nested merge value");
    let mut reference: serde_yaml::Value =
...
target:
  '<<': *base
";
    let mut quoted: Value = saneyaml::from_str(quoted_input).expect("quoted merge key value");
    let mut reference: serde_yaml::Value =
    ...
    let mut tagged_key: Value = saneyaml::from_str(tagged_key_input).expect("tagged key value");
...
";
    ...
    let mut value: Value = saneyaml::from_str(input).expect("nested merge source value");
...
fn serde_api_value_apply_merge_reports_invalid_merge_payloads() {
    ...
    for (input, expected) in cases {
        let error = saneyaml::from_str::<Value>(input).expect_err("invalid default merge payload");
...
fn serde_api_unsigned_numbers_above_i64_are_supported() {
    let max = u64::MAX;
    let value: Value = saneyaml::from_str(&format!("id: {max}\n")).expect("value");
    ...
    let typed: BTreeMap<String, u64> =
        saneyaml::from_str(&format!("id: {max}\n")).expect("typed u64");
...
fn serde_api_value_numeric_predicates_match_serde_yaml() {
    ...
    let value: Value = saneyaml::from_str(input).expect("yaml value");
...
fn serde_api_number_deserializes_directly_and_as_deserializer() {
    let number: Number = saneyaml::from_str("42\n").expect("number from YAML scalar");
...
fn serde_api_target_aware_i128_u128_scalars_match_serde_yaml() {
    for scalar in [
        ...
        let parsed: BTreeMap<String, i128> = saneyaml::from_str(&input)
    ...
    for scalar in [
        ...
        let input = format!("id: {scalar}\n");
        let parsed: BTreeMap<String, u128> = saneyaml::from_str(&input)
...
fn serde_api_to_value_i128_u128_shape_matches_serde_yaml() {
    let in_range_i128 = saneyaml::to_value(i128::from(i64::MAX)).expect("i64 max i128 to_value");
    ...
    let negative_i128 = saneyaml::to_value(i128::from(i64::MIN)).expect("i64 min i128 to_value");
...
fn serde_api_parser_backed_value_preserves_widened_i128_u128_numbers() {
    ...
    let value: Value = saneyaml::from_str(input).expect("parse widened numeric value");
...
fn serde_api_large_integer_string_targets_match_serde_yaml() {
    ...
    let parsed: BTreeMap<String, String> =
        saneyaml::from_str(input).expect("yaml source-backed large integer strings");
    let reference: BTreeMap<String, String> =
...
fn serde_api_i128_u128_range_errors_preserve_scalar_span() {
    for (name, input, expected) in [
        ...
        let error = match name {
            "i128-overflow" => saneyaml::from_str::<BTreeMap<String, i128>>(input)
                .expect_err("i128 overflow rejected"),
            _ => saneyaml::from_str::<BTreeMap<String, u128>>(input)
                .expect_err("u128 range rejected"),
        };
        assert!(
            error.to_string().contains(expected),
...
fn serde_api_from_value_numeric_conversion_matrix() {
    ...
    let as_i64: i64 = saneyaml::from_value(small_unsigned.clone()).expect("small unsigned to i64");
    let as_u64: u64 = saneyaml::from_value(small_unsigned.clone()).expect("small unsigned to u64");
    let as_i128: i128 =
        saneyaml::from_value(small_unsigned.clone()).expect("small unsigned to i128");
    let as_u128: u128 =
        saneyaml::from_value(small_unsigned.clone()).expect("small unsigned to u128");
    let as_f64: f64 = saneyaml::from_value(small_unsigned).expect("small unsigned to f64");
    assert_eq!(as_i64, 5);
    assert_eq!(as_u64, 5);
    assert_eq!(as_i128, 5);
    assert_eq!(as_u128, 5);
    ...
    assert!(saneyaml::from_value::<i64>(large_unsigned.clone()).is_err());
    assert_eq!(
        saneyaml::from_value::<u64>(large_unsigned.clone()).expect("large unsigned to u64"),
        u64::MAX
    );
    assert_eq!(
        saneyaml::from_value::<i128>(large_unsigned.clone()).expect("large unsigned to i128"),
        i128::from(u64::MAX)
    );
    assert_eq!(
        saneyaml::from_value::<u128>(large_unsigned).expect("large unsigned to u128"),
        u128::from(u64::MAX)
    );

    let huge_unsigned = Value::Number(Number::Unsigned(u128::MAX));
    assert!(saneyaml::from_value::<i64>(huge_unsigned.clone()).is_err());
    assert!(saneyaml::from_value::<u64>(huge_unsigned.clone()).is_err());
    assert!(saneyaml::from_value::<i128>(huge_unsigned.clone()).is_err());
    assert_eq!(
        saneyaml::from_value::<u128>(huge_unsigned).expect("huge unsigned to u128"),
        u128::MAX
    );
}
...
fn serde_api_plain_special_floats_match_serde_yaml() {
    ...
    let ours: SpecialFloats = saneyaml::from_str(input).expect("typed special floats");
    ...
    let strings: BTreeMap<String, String> =
        saneyaml::from_str(input).expect("source-backed string targets");
