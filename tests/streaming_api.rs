...
use yaml::{DocumentStream, Event, EventStream, LoadOptions, Node, NodeValue};
...
fn event_stream_collects_exactly_like_parse_events() {
    for input in [
        ...
        let batch = yaml::parse_events(input).expect("batch events");
...
fn event_stream_reader_and_slice_match_string_events() {
    let input = "---\nkind: first\n---\nkind: second\n...\n";
    let expected = yaml::parse_events(input).expect("batch events");
...
flag: true
";
    let batch = yaml::parse_documents(input).expect("batch documents");
...
fn streaming_reader_parse_errors_keep_source_spans() {
    let input = "ok: true\nbad: [unterminated\n";
    let batch_error = yaml::parse_events(input).expect_err("batch parse error");
