...
fn normalize_ours_documents(input: &str, tags: TagPolicy) -> saneyaml::Result<Vec<NormTree>> {
    saneyaml::parse_documents(input).map(|docs| {
        docs.iter()
...
fn normalize_ours_node(node: &saneyaml::Node, tags: TagPolicy) -> NormTree {
    match &node.value {
        saneyaml::NodeValue::Null => NormTree::Null,
        saneyaml::NodeValue::Bool(value) => NormTree::Bool(*value),
        saneyaml::NodeValue::Number(number) => normalize_ours_number(*number),
        saneyaml::NodeValue::String(value) => NormTree::String(value.clone()),
        saneyaml::NodeValue::Sequence(items) => NormTree::Seq(
            items
                .iter()
                .map(|item| normalize_ours_node(item, tags))
                .collect(),
        ),
        saneyaml::NodeValue::Mapping(entries) => NormTree::Map(
            entries
                .iter()
                .map(|(key, value)| {
                    (
        ...
        saneyaml::NodeValue::Tagged(tagged) => match tags {
...
fn normalize_stripped_ours_tagged_node(tagged: &saneyaml::TaggedNode) -> Option<NormTree> {
    if is_yaml_core_tag(&tagged.tag, "str") {
        return Some(normalize_ours_node(&tagged.value, TagPolicy::Strip));
    }

    let value = saneyaml::Value::Tagged(Box::new(saneyaml::TaggedValue {
        tag: tagged.tag.clone(),
        value: saneyaml::Value::from(&tagged.value),
    }));
...
fn is_yaml_core_tag(tag: &saneyaml::Tag, suffix: &str) -> bool {
...
fn normalize_ours_number(number: saneyaml::Number) -> NormTree {
    match number {
        saneyaml::Number::Integer(value) => NormTree::Int(value),
        saneyaml::Number::Unsigned(value) => {
            NormTree::Int(i128::try_from(value).expect("selected parity integer fits i128"))
        }
        saneyaml::Number::Float(value) => NormTree::Float(normalize_float(value)),
    }
}
