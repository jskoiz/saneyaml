...
fn yaml11_collection_tags_deserialize_to_typed_rust_collections() {
    for case in manifest().case.into_iter().filter(|case| {
        ...
        match case.id.as_str() {
            "set-short" => {
                let set: BTreeSet<String> = saneyaml::from_str(&source).expect("short !!set");
                assert_eq!(
                    set,
            ...
            "set-canonical" => {
                let set: BTreeSet<String> = saneyaml::from_str(&source).expect("canonical !!set");
                assert_eq!(
                    set,
            ...
            "omap-short" => {
                let pairs: Vec<(String, i64)> = saneyaml::from_str(&source).expect("short !!omap");
                assert_eq!(
                    pairs,
                    vec![("first".to_string(), 1), ("second".to_string(), 2)]
                );
                let map: BTreeMap<String, i64> =
                    saneyaml::from_str(&source).expect("short !!omap as map");
                assert_eq!(
                    map,
            ...
            "pairs-short" => {
                let pairs: Vec<(String, i64)> = saneyaml::from_str(&source).expect("short !!pairs");
                assert_eq!(
                    pairs,
            ...
            "resolved-tag-handle" => {
                let bundle: ResolvedBundle =
                    saneyaml::from_str(&source).expect("resolved %TAG bundle");
                assert_eq!(
                    bundle.set,
            ...
            "custom-handle-tags" => {
                let bundle: CustomHandleBundle =
                    saneyaml::from_str(&source).expect("custom %TAG bundle");
                assert_eq!(
                    bundle.set,
                ...
                let value: Value = saneyaml::from_str(&source).expect("custom %TAG retained value");
                for (key, suffix, shape) in [
                    ...
                    assert_tagged_child_payload(
                        &value,
                        key,
                        saneyaml::Tag::new(format!("!<tag:yaml.org,2002:{suffix}>")),
                        shape,
                    );
                }
            }
...
fn yaml11_core_structural_tags_are_retained_and_transparent_for_typed_reads() {
    ...
    let default: CoreStructuralTags = saneyaml::from_str(&source).expect("default structural tags");
    ...
    assert_tagged_child_payload(&value, "short_seq", saneyaml::Tag::new("!!seq"), "sequence");
    assert_tagged_child_payload(
        &value,
        "canonical_seq",
    ...
    assert_tagged_child_payload(&value, "short_map", saneyaml::Tag::new("!!map"), "mapping");
    assert_tagged_child_payload(
        &value,
        "canonical_map",
        saneyaml::Tag::new("!<tag:yaml.org,2002:map>"),
        "mapping",
    );
    assert_tagged_child_payload(
        &value,
    ...
    assert_eq!(tagged_value.tag, saneyaml::Tag::new("!!value"));
    ...
    let lossless = saneyaml::parse_lossless(&source).expect("structural tags parse losslessly");
...
fn yaml11_value_structural_tag_handles_and_duplicate_keys_are_explicit() {
    ...
    assert_tagged_child_payload(
        &value,
        "value_key",
        saneyaml::Tag::new("!<tag:yaml.org,2002:value>"),
        "scalar",
    );
    assert_tagged_key(
        &value["value_mapping"],
...
fn yaml11_collection_tags_reject_lossy_typed_shapes() {
    ...
    let error = saneyaml::from_str::<BTreeSet<String>>(&set_source)
    ...
    let error = saneyaml::from_str::<Vec<(String, i64)>>(&omap_source)
    ...
    let error = saneyaml::from_str::<Vec<(String, i64)>>(&pairs_source)
...
fn yaml11_legacy_migration_pack_covers_default_explicit_and_directive_modes() {
    ...
    let default: Value = saneyaml::from_str(&source).expect("default YAML 1.2-oriented value");
...
fn yaml11_legacy_scalar_denominator_covers_public_entrypoints() {
    ...
    let default: Value = saneyaml::from_str(&source).expect("default scalar denominator value");
...
fn yaml11_legacy_merge_fixture_expands_after_directive_schema_resolution() {
    ...
    let default: Value = saneyaml::from_str(&source).expect("default merge fixture");
...
fn yaml11_explicit_merge_tag_fixture_expands_and_keeps_literal_tags() {
    ...
    let value: Value = saneyaml::from_str(&source).expect("explicit merge-tag fixture");
...
fn yaml11_legacy_merge_edge_fixture_recovers_like_psych() {
    ...
    let default_error = saneyaml::parse_str(&source)
...
fn yaml11_lossless_graph_merge_fixtures_preserve_source_graph_and_semantic_policy() {
    ...
    {
        let source = read_fixture(&case.path);
        saneyaml::parse_events(&source)
            .unwrap_or_else(|error| panic!("{} raw events parse: {error}", case.id));

        let lossless = saneyaml::parse_lossless(&source)
        ...
        match case.id.as_str() {
            "lossless-merge-graph" => {
                ...
                let default: Value =
                    saneyaml::from_str(&source).expect("default lossless merge graph fixture");
...
fn yaml11_explicit_merge_tag_bad_payload_fixture_reports_scalar_span() {
    ...
    let error = saneyaml::parse_str(&source).expect_err("invalid explicit merge-tag payload");
    assert!(
        error
            .to_string()
    ...
    let value_error =
        saneyaml::from_str::<Value>(&source).expect_err("Value read rejects invalid merge payload");
...
fn yaml11_legacy_bool_key_collision_fixture_keeps_default_safe_and_reports_legacy_span() {
    ...
    let default: Value =
        saneyaml::from_str(&source).expect("default keeps bool-like keys as strings");
...
fn yaml11_legacy_scalar_edge_stream_switches_per_document() {
    ...
    let lossless = saneyaml::parse_lossless(&source).expect("YAML 1.1 stream parses losslessly");
    assert_eq!(lossless.as_source(), source);
    assert_eq!(lossless.to_string(), source);
...
fn yaml11_directive_boundary_stream_resets_schema_and_tag_handles() {
    ...
    assert_eq!(
        docs[0]["tagged"]
            ...
            .tag,
        saneyaml::Tag::new("!<tag:yaml.org,2002:str>")
    );
    ...
    let lossless = saneyaml::parse_lossless(&source).expect("directive boundary lossless parse");
    assert_eq!(lossless.as_source(), source);
    assert_eq!(lossless.to_string(), source);
...
";
    for (label, result) in [
        (
            "events",
            saneyaml::parse_events(tag_scope_reset).map(|_| ()),
        ),
        (
            "documents",
            LoadOptions::yaml_version_directive()
        ...
        (
            "lossless",
            saneyaml::parse_lossless(tag_scope_reset).map(|_| ()),
        ),
    ] {
...
value
";
    let error = saneyaml::parse_events(duplicate_tag).expect_err("duplicate TAG handle rejected");
    assert!(
...
";
    let error =
        saneyaml::parse_events(duplicate_yaml).expect_err("duplicate YAML directive rejected");
    assert!(
...
fn yaml11_legacy_flow_scalar_fixture_switches_inside_flow_collections() {
    ...
    let default: Value = saneyaml::from_str(&source).expect("default flow scalar fixture");
...
fn yaml11_legacy_numeric_key_collision_fixture_keeps_default_safe() {
    ...
    let default: Value =
        saneyaml::from_str(&source).expect("default keeps decimal key identity safe");
...
fn yaml11_legacy_signed_zero_key_collision_fixture_reports_span() {
    ...
    let default = saneyaml::from_str::<Value>(&source)
...
fn yaml11_legacy_alias_key_collision_fixture_keeps_default_safe() {
    ...
    let default: Value =
        saneyaml::from_str(&source).expect("default keeps yes alias and true key distinct");
    assert_eq!(
        default["root"]
            .as_mapping()
...
fn assert_tagged_payload(source: &str, handle: &str, suffix: &str, shape: &str) {
    let value: Value = saneyaml::from_str(source).expect("tagged collection value");
...
fn assert_tagged_child_payload(value: &Value, key: &str, tag: saneyaml::Tag, shape: &str) {
...
fn assert_tagged_key(mapping: &Value, tag: saneyaml::Tag, key: &str, expected: &str) {
...
fn assert_legacy_pack_public_entrypoints(
    ...
    let from_node: LegacyMigrationPack = saneyaml::from_node(&parsed_str)
        .unwrap_or_else(|error| panic!("{label} from_node: {error}"));
    let from_value: LegacyMigrationPack = saneyaml::from_value(Value::from(&parsed_str))
...
fn assert_scalar_denominator_public_entrypoints(
    ...
    let from_node: LegacyScalarDenominator = saneyaml::from_node(&parsed_str)
        .unwrap_or_else(|error| panic!("{label} scalar denominator from_node: {error}"));
    let from_value: LegacyScalarDenominator = saneyaml::from_value(Value::from(&parsed_str))
