...
fn assert_manifest_error(case: &SuiteCase, entrypoint: &str, error: &saneyaml::Error) {
...
fn yts_manifest_acceptance_policy_matches_parser_event_and_serde_entrypoints() {
    ...
    for case in &manifest.case {
        ...
        if case.is_error_case() {
            ...
            let error = match saneyaml::from_documents_str::<saneyaml::Value>(&input) {
                Err(error) => error,
                Ok(_) => panic!(
        ...
        saneyaml::from_documents_str::<saneyaml::Value>(&input).unwrap_or_else(|error| {
            panic!(
    ...
    assert!(events.iter().any(|event| {
        matches!(
            ...
            Event::Scalar {
                meta,
                ..
            } if meta.tag.as_ref().is_some_and(|tag| tag.tag == saneyaml::Tag::new("!"))
        )
    }));
}
...
fn assert_various_trailing_comments_tree(name: &str, doc: &saneyaml::Node) {
    let Value::Mapping(entries) = &doc.value else {
...
fn yts_events_s4jq__preserve_explicit_non_specific_tag() {
    ...
    assert_eq!(scalars[2].2, Some(&saneyaml::Tag::new("!")));
}
            ...
            Event::MappingStart { meta, .. }
                if meta.anchor.as_ref().is_some_and(|anchor| {
                    ...
                    && meta.tag.as_ref().is_some_and(|tag| {
                        tag.tag == saneyaml::Tag::new("!!map") && tag.span.line == 2
                    })
        )
    }));
}
...
fn yts_parse_bec7_mus6__yaml_version_directive_variants_are_syntax_only() {
    for (name, input, version) in [
        ...
        let values: Vec<saneyaml::Value> = saneyaml::from_documents_str(input)
    ...
    let saneyaml::NodeValue::Mapping(entries) = doc.value else {
        panic!("expected mapping");
    };
    ...
    let error = saneyaml::LoadOptions::yaml_version_directive()
        .parse_str("%YAML 1.1\n---\non: off\nyes: no\n")
...
fn yts_parse_9kax__tag_anchor_property_combinations() {
    ...
    assert_eq!(tagged_map.tag, saneyaml::Tag::new("!!map"));
    ...
    assert_eq!(tagged_key.tag, saneyaml::Tag::new("!!str"));
            ...
            Event::MappingStart { meta, .. }
                if meta.anchor.as_ref().is_some_and(|anchor| anchor.name == "a4")
                    && meta
                        .tag
                        .as_ref()
                        .is_some_and(|tag| tag.tag == saneyaml::Tag::new("!!map"))
        )
    }));
...
fn yts_parse_6ca3__tab_before_root_flow_sequence() {
    ...
    assert!(events.iter().any(|event| {
        matches!(
            event,
            Event::SequenceStart {
                style: saneyaml::CollectionStyle::Flow,
                ..
            }
        )
    }));
...
fn yts_parse_57h4__block_collection_nodes() {
    ...
    assert_eq!(nested.tag, saneyaml::Tag::new("!!seq"));
...
fn yts_events_57h4__block_collection_node_tags() {
    ...
    let tagged_sequences = events
        ...
        .filter(|event| {
            matches!(
                event,
                Event::SequenceStart { meta, .. }
                    if meta.tag.as_ref().is_some_and(|tag| tag.tag == saneyaml::Tag::new("!!seq"))
            )
        })
        .count();
    ...
    assert!(events.iter().any(|event| {
        matches!(
            event,
            Event::MappingStart { meta, .. }
                if meta.tag.as_ref().is_some_and(|tag| tag.tag == saneyaml::Tag::new("!!map"))
        )
    }));
}
...
fn yts_events_m5c3__separated_tag_on_block_scalar_spans() {
    ...
    assert_eq!(tag.tag, saneyaml::Tag::new("foo"));
